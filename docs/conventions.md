...
- **Fallible op:** `[[nodiscard]] tda_Status foo(args…, T *out);`. `[[nodiscard]]` turns an
  ignored error into a compile error; `out` is written **only on `TDA_STATUS_OK`**, left
  untouched otherwise. Propagate by hand — `tda_Status st = foo(…); if
  (TDA_STATUS_IS_ERR(st)) return st;` — or `goto fail` while resources are held, since C
