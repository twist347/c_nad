...
static_assert(BITINT_MAXWIDTH >= 128, "trs_rng_u64_max needs a 128-bit product");
...
[[nodiscard]]
static uint64_t next_u64(trs_Rng *self);
...
size_t trs_rng_idx(trs_Rng *self, size_t len) {
    ...
    return (size_t) trs_rng_u64_max(self, (uint64_t) len - 1);
}
...
int32_t trs_rng_i32_range(trs_Rng *self, int32_t lo, int32_t hi) {
    ...
    return (int32_t) ((uint32_t) lo + trs_rng_u32_max(self, width));
}
...
int64_t trs_rng_i64_range(trs_Rng *self, int64_t lo, int64_t hi) {
    ...
    return (int64_t) ((uint64_t) lo + trs_rng_u64_max(self, width));
}
...
float trs_rng_f32_range(trs_Rng *self, float lo, float hi) {
    ...
    const float val = lo + (hi - lo) * trs_rng_f32(self);
...
double trs_rng_f64_range(trs_Rng *self, double lo, double hi) {
    ...
    const double val = lo + (hi - lo) * trs_rng_f64(self);
