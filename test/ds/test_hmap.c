...
static trs_Hash hash_by_parity(const void *x) {
    return (trs_Hash) (*(const int32_t *) x & 1);
}
...
static trs_Hash hash_high_bits_only(const void *x) {
    return ((trs_Hash) *(const int32_t *) x) << 32;
}

static size_t hash_calls = 0;

static trs_Hash hash_i32_counting(const void *x) {
    ++hash_calls;

    return trs_hash_i32(x);
}
...
static bool eq_i32_counting(const void *lhs, const void *rhs) {
    ...
    return trs_eq_i32(lhs, rhs);
}

[[nodiscard]]
static trs_HMap *make_map(trs_Hasher hasher) {
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, hasher, trs_eq_i32, trs_al_default(), &m));

    return m;
}

static void put(trs_HMap *m, int32_t key, int32_t val) {
    TRS_TEST_OK(TRS_HMAP_INSERT(int32_t, int32_t, m, key, val, nullptr));
}

// 'key * 10' under every key from 0 to n - 1
[[nodiscard]]
static trs_HMap *make_filled(trs_Hasher hasher, int32_t n) {
    trs_HMap *m = make_map(hasher);
...
static void assert_has(const trs_HMap *m, int32_t key, int32_t val) {
    const int32_t *got = TRS_HMAP_GET_AS(int32_t, int32_t, m, key);
    TEST_ASSERT_NOT_NULL_MESSAGE(got, "the key is missing");
    TEST_ASSERT_EQUAL_INT32(val, *got);
    TEST_ASSERT_TRUE(TRS_HMAP_CONTAINS(int32_t, m, key));
}

static void assert_missing(const trs_HMap *m, int32_t key) {
    TEST_ASSERT_NULL(TRS_HMAP_GET_AS(int32_t, int32_t, m, key));
    TEST_ASSERT_FALSE(TRS_HMAP_CONTAINS(int32_t, m, key));
    TEST_ASSERT_NULL(TRS_HMAP_FIND(int32_t, m, key));
}
...
static void assert_walk_sees_everything(const trs_HMap *m) {
    size_t seen = 0;
    TRS_HMAP_FOR_EACH(node, m) {
        const int32_t key = *TRS_HMAP_NODE_KEY_AS(int32_t, node);
        const int32_t val = *TRS_HMAP_NODE_VAL_AS(int32_t, m, node);
        assert_has(m, key, val);
        ++seen;
    }
    TEST_ASSERT_EQUAL_size_t(trs_hmap_len(m), seen);
}
...
static void test_new_starts_empty() {
    ...
    TEST_ASSERT_EQUAL_size_t(0, trs_hmap_len(m));
    TEST_ASSERT_EQUAL_size_t(0, trs_hmap_bucket_count(m));
    TEST_ASSERT_EQUAL_size_t(sizeof(int32_t), trs_hmap_key_size(m));
    TEST_ASSERT_EQUAL_size_t(sizeof(int32_t), trs_hmap_val_size(m));
    TEST_ASSERT_EQUAL_PTR(trs_al_default(), trs_hmap_al(m));
    TEST_ASSERT_EQUAL_PTR(trs_hash_i32, trs_hmap_hasher(m));
    TEST_ASSERT_EQUAL_PTR(trs_eq_i32, trs_hmap_key_eq(m));
    TEST_ASSERT_NULL(trs_hmap_first_node(m));
...
static void test_new_cap_reserves_buckets_without_entries() {
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW_CAP(int32_t, int32_t, 100, trs_hash_i32, trs_eq_i32, trs_al_default(), &m));

    TEST_ASSERT_EQUAL_size_t(0, trs_hmap_len(m));
    TEST_ASSERT_TRUE(trs_hmap_bucket_count(m) >= 100);

    trs_hmap_drop(m);
}

static void test_drop_null_is_noop() {
    trs_hmap_drop(nullptr);
}
...
static void test_insert_then_get() {
    ...
    assert_has(m, 1, 10);
    assert_has(m, 2, 20);
    TEST_ASSERT_EQUAL_size_t(2, trs_hmap_len(m));
...
static void test_insert_overwrites_an_existing_key() {
    ...
    assert_has(m, 1, 99);
    TEST_ASSERT_EQUAL_size_t(1, trs_hmap_len(m)); // an overwrite is not a second entry
...
static void test_insert_reports_whether_the_key_was_new() {
    ...
    bool is_new = false;
    TRS_TEST_OK(TRS_HMAP_INSERT(int32_t, int32_t, m, 1, 10, &is_new));
    TEST_ASSERT_TRUE(is_new);

    TRS_TEST_OK(TRS_HMAP_INSERT(int32_t, int32_t, m, 1, 20, &is_new));
    TEST_ASSERT_FALSE(is_new);

    TRS_TEST_OK(TRS_HMAP_INSERT(int32_t, int32_t, m, 2, 20, &is_new));
    TEST_ASSERT_TRUE(is_new);
...
static void test_get_mut_writes_through() {
    ...
    *TRS_HMAP_GET_MUT_AS(int32_t, int32_t, m, 1) = 42;
...
static void test_find_gives_the_entry_as_a_node() {
    ...
    const trs_HMapNode *node = TRS_HMAP_FIND(int32_t, m, 7);
    TEST_ASSERT_NOT_NULL(node);
    TEST_ASSERT_EQUAL_INT32(7, *TRS_HMAP_NODE_KEY_AS(int32_t, node));
    TEST_ASSERT_EQUAL_INT32(70, *TRS_HMAP_NODE_VAL_AS(int32_t, m, node));
...
static void test_a_map_whose_keys_all_collide_still_finds_them() {
    trs_HMap *m = make_filled(hash_all_alike, 20);

    TEST_ASSERT_EQUAL_size_t(20, trs_hmap_len(m));
    for (int32_t i = 0; i < 20; ++i) {
        assert_has(m, i, i * 10);
    }
    assert_missing(m, 20);
...
static void test_colliding_keys_overwrite_only_their_own_entry() {
    trs_HMap *m = make_filled(hash_all_alike, 5);

    put(m, 3, 999);

    TEST_ASSERT_EQUAL_size_t(5, trs_hmap_len(m));
    assert_has(m, 0, 0);
    assert_has(m, 3, 999);
    assert_has(m, 4, 40);
...
static void test_a_hash_collision_is_not_an_equality() {
    trs_HMap *m = make_map(hash_all_alike);
...
static void test_wide_keys_and_values_travel_whole() {
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(Pair, Pair, hash_pair, eq_pair, trs_al_default(), &m));

    constexpr Pair key = {1, 2};
    constexpr Pair val = {30, 40};
    TRS_TEST_OK(trs_hmap_insert(m, &key, &val, nullptr));

    const Pair *got = trs_hmap_get(m, &key);
    TEST_ASSERT_NOT_NULL(got);
    TEST_ASSERT_EQUAL_INT64(30, got->a);
    TEST_ASSERT_EQUAL_INT64(40, got->b);
    TEST_ASSERT_EQUAL_size_t(sizeof(Pair), trs_hmap_key_size(m));
...
static void test_the_chain_is_walked_by_hash_before_eq() {
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, hash_high_bits_only, eq_i32_counting, trs_al_default(), &m));

    for (int32_t i = 0; i < 10; ++i) {
        put(m, i, i * 10);
    }
    eq_calls = 0;
    const int32_t *got = TRS_HMAP_GET_AS(int32_t, int32_t, m, 7);
    TEST_ASSERT_NOT_NULL(got);
...
static void test_the_buckets_double_as_the_map_fills() {
    ...
    put(m, 0, 0);
    const size_t first = trs_hmap_bucket_count(m);
    ...
    TEST_ASSERT_TRUE(trs_hmap_bucket_count(m) > first);
    TEST_ASSERT_TRUE(trs_hmap_len(m) <= trs_hmap_bucket_count(m));

    trs_hmap_drop(m);
}

static void test_every_entry_survives_the_growths() {
    trs_HMap *m = make_filled(trs_hash_i32, 300);

    TEST_ASSERT_EQUAL_size_t(300, trs_hmap_len(m));
...
static void test_a_borrowed_node_survives_every_growth() {
    ...
    const trs_HMapNode *held[4];
    for (int32_t i = 0; i < 4; ++i) {
        held[i] = TRS_HMAP_FIND(int32_t, m, i);
    ...
    for (int32_t i = 0; i < 4; ++i) {
        TEST_ASSERT_EQUAL_PTR(held[i], TRS_HMAP_FIND(int32_t, m, i));
        TEST_ASSERT_EQUAL_INT32(i, *TRS_HMAP_NODE_KEY_AS(int32_t, held[i]));
        TEST_ASSERT_EQUAL_INT32(i * 10, *TRS_HMAP_NODE_VAL_AS(int32_t, m, held[i]));
    }
...
static void test_a_growth_allocates_only_the_bucket_array() {
    ...
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW_CAP(int32_t, int32_t, 8, trs_hash_i32, trs_eq_i32, &al, &m));
    ...
    TEST_ASSERT_EQUAL_size_t(2, trs_test_probe_requests(&probe) - before);
...
static void test_reserve_grows_the_buckets_only() {
    trs_HMap *m = make_filled(trs_hash_i32, 4);

    TRS_TEST_OK(trs_hmap_reserve(m, 1000));

    TEST_ASSERT_TRUE(trs_hmap_bucket_count(m) >= 1000);
    TEST_ASSERT_EQUAL_size_t(4, trs_hmap_len(m));
    for (int32_t i = 0; i < 4; ++i) {
        assert_has(m, i, i * 10);
    }
...
static void test_reserve_below_the_bucket_count_changes_nothing() {
    trs_HMap *m = make_filled(trs_hash_i32, 20);
    const size_t before = trs_hmap_bucket_count(m);

    TRS_TEST_OK(trs_hmap_reserve(m, 1));

    TEST_ASSERT_EQUAL_size_t(before, trs_hmap_bucket_count(m));
...
static void test_remove_takes_the_entry_out() {
    trs_HMap *m = make_filled(trs_hash_i32, 5);

    TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, m, 2));
...
static void test_remove_of_a_missing_key_says_so() {
    trs_HMap *m = make_filled(trs_hash_i32, 3);

    TEST_ASSERT_FALSE(TRS_HMAP_REMOVE(int32_t, m, 99));
    TEST_ASSERT_EQUAL_size_t(3, trs_hmap_len(m));
...
static void test_remove_on_an_empty_map_says_so() {
    ...
    TEST_ASSERT_FALSE(TRS_HMAP_REMOVE(int32_t, m, 1));
...
static void test_remove_every_key_of_a_chain_in_turn() {
    trs_HMap *m = make_filled(hash_all_alike, 6);

    for (int32_t i = 0; i < 6; ++i) {
        TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, m, i));
        TEST_ASSERT_EQUAL_size_t((size_t) (5 - i), trs_hmap_len(m));
    }
...
static void test_mut_walk_writes_through_every_entry() {
    ...
    size_t seen = 0;
    for (trs_HMapNode *node = trs_hmap_first_node_mut(m); node; node = trs_hmap_node_next_mut(m, node)) {
        *TRS_HMAP_NODE_VAL_MUT_AS(int32_t, m, node) *= 2;
        ++seen;
    }
    TEST_ASSERT_EQUAL_size_t(4, seen);

    for (int32_t key = 0; key < 4; ++key) {
        TEST_ASSERT_EQUAL_INT32(key * 20, *TRS_HMAP_GET_AS(int32_t, int32_t, m, key));
    }
...
static void test_mut_walk_of_an_empty_map_stops_at_once() {
    ...
    TEST_ASSERT_NULL(trs_hmap_first_node_mut(m));

    trs_hmap_drop(m);
}

static void test_remove_node_drops_the_entry_it_names() {
    trs_HMap *m = make_filled(hash_all_alike, 4);

    trs_HMapNode *node = trs_hmap_find_mut(m, &(int32_t){2});
    TEST_ASSERT_NOT_NULL(node);
...
static void test_a_key_can_be_put_back_after_removal() {
    ...
    TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, m, 1));
    put(m, 1, 111);
...
static void test_clear_empties_and_keeps_the_buckets() {
    trs_HMap *m = make_filled(trs_hash_i32, 20);
    const size_t buckets = trs_hmap_bucket_count(m);

    trs_hmap_clear(m);

    TEST_ASSERT_EQUAL_size_t(0, trs_hmap_len(m));
    TEST_ASSERT_EQUAL_size_t(buckets, trs_hmap_bucket_count(m));
    TEST_ASSERT_NULL(trs_hmap_first_node(m));
    assert_missing(m, 1);
...
static void test_clear_leaves_a_usable_map() {
    trs_HMap *m = make_filled(trs_hash_i32, 10);

    trs_hmap_clear(m);
    put(m, 5, 55);

    TEST_ASSERT_EQUAL_size_t(1, trs_hmap_len(m));
    assert_has(m, 5, 55);
...
static void test_the_walk_reaches_every_entry_once() {
    trs_HMap *m = make_filled(trs_hash_i32, 50);
...
static void test_the_walk_crosses_a_single_chain() {
    trs_HMap *m = make_filled(hash_all_alike, 12);
...
static void test_the_walk_skips_the_empty_buckets() {
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW_CAP(int32_t, int32_t, 64, hash_by_parity, trs_eq_i32, trs_al_default(), &m));
    for (int32_t i = 0; i < 10; ++i) {
        put(m, i, i * 10);
    }

    TEST_ASSERT_TRUE(trs_hmap_bucket_count(m) >= 64);
    assert_walk_sees_everything(m);
...
static void test_the_walk_of_an_empty_map_stops_at_once() {
    ...
    trs_HMap *cleared = make_filled(trs_hash_i32, 5);
    trs_hmap_clear(cleared);
    TEST_ASSERT_NULL(trs_hmap_first_node(cleared));

    trs_hmap_drop(m);
    trs_hmap_drop(cleared);
}

static void test_the_walk_after_a_removal_sees_the_rest() {
    trs_HMap *m = make_filled(trs_hash_i32, 30);

    for (int32_t i = 0; i < 30; i += 2) {
        TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, m, i));
    }

    TEST_ASSERT_EQUAL_size_t(15, trs_hmap_len(m));
    assert_walk_sees_everything(m);
...
static void test_get_or_insert_puts_a_missing_key_in() {
    ...
    trs_HMapNode *node = nullptr;
    TRS_TEST_OK(TRS_HMAP_GET_OR_INSERT(int32_t, int32_t, m, 1, 10, &node));

    TEST_ASSERT_NOT_NULL(node);
    TEST_ASSERT_EQUAL_INT32(1, *TRS_HMAP_NODE_KEY_AS(int32_t, node));
    TEST_ASSERT_EQUAL_INT32(10, *TRS_HMAP_NODE_VAL_AS(int32_t, m, node));
    TEST_ASSERT_EQUAL_size_t(1, trs_hmap_len(m));
    assert_has(m, 1, 10);
...
static void test_get_or_insert_leaves_a_present_key_alone() {
    ...
    trs_HMapNode *node = nullptr;
    TRS_TEST_OK(TRS_HMAP_GET_OR_INSERT(int32_t, int32_t, m, 1, 999, &node));
...
static void test_get_or_insert_hands_back_the_node_that_is_already_there() {
    trs_HMap *m = make_filled(hash_all_alike, 5);
    const trs_HMapNode *held = TRS_HMAP_FIND(int32_t, m, 3);

    trs_HMapNode *node = nullptr;
    TRS_TEST_OK(TRS_HMAP_GET_OR_INSERT(int32_t, int32_t, m, 3, 0, &node));
...
static void test_get_or_insert_hashes_the_key_once() {
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, hash_i32_counting, trs_eq_i32, trs_al_default(), &m));
    ...
    hash_calls = 0;
    if (!trs_hmap_get_mut(m, &(int32_t){2})) {
        TRS_TEST_OK(TRS_HMAP_INSERT(int32_t, int32_t, m, 2, 20, nullptr));
    }
    TEST_ASSERT_EQUAL_size_t(2, hash_calls);

    // and once again on a key that IS there, where both forms cost the same
    hash_calls = 0;
    TRS_TEST_OK(TRS_HMAP_GET_OR_INSERT(int32_t, int32_t, m, 1, 0, &node));
    TEST_ASSERT_EQUAL_size_t(1, hash_calls);
...
static void test_get_or_insert_carries_the_counter_idiom() {
    ...
    for (size_t i = 0; i < 6; ++i) {
        trs_HMapNode *node = nullptr;
        TRS_TEST_OK(trs_hmap_get_or_insert(m, &seen[i], &(int32_t){ 0 }, &node));
        ++*TRS_HMAP_NODE_VAL_MUT_AS(int32_t, m, node);
    }
...
static void test_shrink_to_fit_gives_the_buckets_back() {
    trs_HMap *m = make_filled(trs_hash_i32, 300);
    const size_t grown = trs_hmap_bucket_count(m);

    for (int32_t i = 3; i < 300; ++i) {
        TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, m, i));
    }

    TRS_TEST_OK(trs_hmap_shrink_to_fit(m));

    TEST_ASSERT_TRUE(trs_hmap_bucket_count(m) < grown);
    TEST_ASSERT_TRUE(trs_hmap_bucket_count(m) >= trs_hmap_len(m));
...
static void test_shrink_to_fit_stops_at_the_starting_size() {
    trs_HMap *fresh = make_map(trs_hash_i32);
    put(fresh, 0, 0);
    const size_t base = trs_hmap_bucket_count(fresh);
    ...
    TEST_ASSERT_EQUAL_size_t(base, trs_hmap_bucket_count(m));
    assert_has(m, 0, 0);
...
static void test_shrink_to_fit_keeps_borrowed_nodes() {
    trs_HMap *m = make_filled(trs_hash_i32, 200);

    const trs_HMapNode *held[3];
    for (int32_t i = 0; i < 3; ++i) {
        held[i] = TRS_HMAP_FIND(int32_t, m, i);
    }
...
static void test_a_shrunk_map_grows_again() {
    trs_HMap *m = make_filled(trs_hash_i32, 100);
    for (int32_t i = 2; i < 100; ++i) {
        TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, m, i));
    }
    TRS_TEST_OK(trs_hmap_shrink_to_fit(m));
    ...
    TEST_ASSERT_EQUAL_size_t(100, trs_hmap_len(m));
    for (int32_t i = 0; i < 100; ++i) {
        assert_has(m, i, i * 10);
    }
...
static void test_copy_is_independent() {
    trs_HMap *src = make_filled(trs_hash_i32, 10);

    trs_HMap *dst = nullptr;
    TRS_TEST_OK(trs_hmap_copy(src, &dst));

    put(dst, 0, 999);
    TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, dst, 1));
...
static void test_copy_carries_the_hasher_and_the_equality() {
    ...
    trs_HMap *src = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, hash_all_alike, trs_eq_i32, arena, &src));
    put(src, 1, 10);

    trs_HMap *dst = nullptr;
    TRS_TEST_OK(trs_hmap_copy(src, &dst));

    TEST_ASSERT_EQUAL_PTR(hash_all_alike, trs_hmap_hasher(dst));
    TEST_ASSERT_EQUAL_PTR(trs_eq_i32, trs_hmap_key_eq(dst));
    TEST_ASSERT_EQUAL_PTR(arena, trs_hmap_al(dst));
    assert_has(dst, 1, 10);
...
static void test_copy_with_builds_on_the_given_allocator() {
    ...
    trs_HMap *src = make_filled(trs_hash_i32, 8);

    trs_HMap *dst = nullptr;
    TRS_TEST_OK(trs_hmap_copy_with(src, arena, &dst));

    TEST_ASSERT_EQUAL_PTR(arena, trs_hmap_al(dst));
    TEST_ASSERT_EQUAL_PTR(trs_al_default(), trs_hmap_al(src));
    TEST_ASSERT_EQUAL_PTR(trs_hash_i32, trs_hmap_hasher(dst));
    TEST_ASSERT_EQUAL_PTR(trs_eq_i32, trs_hmap_key_eq(dst));
    TEST_ASSERT_TRUE(trs_hmap_eq(src, dst));
...
static void test_copy_with_reports_an_exhausted_target_arena() {
    ...
    trs_HMap *dst = nullptr;
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_hmap_copy_with(src, arena, &dst));
    TEST_ASSERT_NULL(dst);
    TEST_ASSERT_EQUAL_size_t(8, trs_hmap_len(src));
...
static void test_move_assign_hands_over_the_contents_on_one_allocator() {
    ...
    trs_HMap *src = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, hash_all_alike, trs_eq_i32, &al, &src));
    put(src, 1, 10);
    put(src, 2, 20);

    trs_HMap *dst = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, trs_hash_i32, trs_eq_i32, &al, &dst));
    put(dst, 9, 90);

    const size_t requests = trs_test_probe_requests(&probe);
    TRS_TEST_OK(trs_hmap_move_assign(src, dst));
    ...
    TEST_ASSERT_EQUAL_size_t(2, trs_hmap_len(dst));
    assert_has(dst, 1, 10);
    assert_has(dst, 2, 20);
    ...
    TEST_ASSERT_EQUAL_size_t(0, trs_hmap_len(src));
...
static void test_move_assign_across_allocators_empties_the_source() {
    ...
    trs_HMap *dst = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, hash_all_alike, trs_eq_i32, arena, &dst));
    put(dst, 9, 90);

    TRS_TEST_OK(trs_hmap_move_assign(src, dst));

    TEST_ASSERT_EQUAL_size_t(8, trs_hmap_len(dst));
    assert_has(dst, 3, 30);
...
static void test_move_assign_across_allocators_reports_an_exhausted_arena() {
    ...
    trs_HMap *dst = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, trs_hash_i32, trs_eq_i32, arena, &dst));
    put(dst, 9, 90);
    trs_test_arena_leave(arena, 0);

    trs_HMap *src = make_filled(trs_hash_i32, 8);

    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_hmap_move_assign(src, dst));

    TEST_ASSERT_EQUAL_size_t(8, trs_hmap_len(src));
    TEST_ASSERT_EQUAL_size_t(1, trs_hmap_len(dst));
    assert_has(dst, 9, 90);
...
static void test_move_assign_of_itself_changes_nothing() {
    ...
    TRS_TEST_OK(trs_hmap_move_assign(m, m));
...
static void test_copy_of_empty_stays_empty() {
    trs_HMap *src = make_map(trs_hash_i32);

    trs_HMap *dst = nullptr;
    TRS_TEST_OK(trs_hmap_copy(src, &dst));

    TEST_ASSERT_EQUAL_size_t(0, trs_hmap_len(dst));
...
static void test_copy_assign_overwrites_the_target() {
    trs_HMap *src = make_filled(trs_hash_i32, 6);
    trs_HMap *dst = make_filled(trs_hash_i32, 2);
    put(dst, 100, 100);

    TRS_TEST_OK(trs_hmap_copy_assign(src, dst));

    TEST_ASSERT_EQUAL_size_t(6, trs_hmap_len(dst));
...
static void test_copy_assign_hands_over_the_hasher_too() {
    trs_HMap *src = make_map(hash_all_alike);
    put(src, 1, 10);

    trs_HMap *dst = make_map(trs_hash_i32);
    put(dst, 2, 20);
...
static void test_copy_assign_self_is_noop() {
    ...
    TRS_TEST_OK(trs_hmap_copy_assign(m, m));
...
static void test_copy_assign_keeps_the_target_allocator() {
    ...
    trs_HMap *src = make_filled(trs_hash_i32, 4);
    ...
    TEST_ASSERT_EQUAL_PTR(arena, trs_hmap_al(dst));
    TEST_ASSERT_EQUAL_size_t(4, trs_hmap_len(dst));
...
static void test_copy_assign_hands_back_the_old_contents() {
    ...
    trs_HMap *src = make_filled(trs_hash_i32, 3);
    TRS_TEST_OK(trs_hmap_copy_assign(src, dst));

    TEST_ASSERT_EQUAL_size_t(3, trs_hmap_len(dst));
...
static void test_swap_exchanges_the_entries_and_the_hashers() {
    trs_HMap *a = make_map(trs_hash_i32);
    put(a, 1, 10);

    trs_HMap *b = make_map(hash_all_alike);
    put(b, 2, 20);
    put(b, 3, 30);

    trs_hmap_swap(a, b);

    TEST_ASSERT_EQUAL_size_t(2, trs_hmap_len(a));
    TEST_ASSERT_EQUAL_size_t(1, trs_hmap_len(b));
    TEST_ASSERT_EQUAL_PTR(hash_all_alike, trs_hmap_hasher(a));
    TEST_ASSERT_EQUAL_PTR(trs_hash_i32, trs_hmap_hasher(b));
    assert_has(a, 2, 20);
    assert_has(b, 1, 10);
...
static void test_swap_keeps_the_nodes_alive() {
    trs_HMap *a = make_filled(trs_hash_i32, 4);
    trs_HMap *b = make_map(trs_hash_i32);

    const trs_HMapNode *held = TRS_HMAP_FIND(int32_t, a, 2);
    TEST_ASSERT_NOT_NULL(held);

    trs_hmap_swap(a, b);

    TEST_ASSERT_EQUAL_PTR(held, TRS_HMAP_FIND(int32_t, b, 2));
...
static void test_new_reports_an_exhausted_arena() {
    ...
    trs_HMap *m = nullptr;
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, TRS_HMAP_NEW(int32_t, int32_t, trs_hash_i32, trs_eq_i32, arena, &m));
    TEST_ASSERT_NULL(m);
...
static void test_new_cap_frees_the_header_when_the_buckets_are_refused() {
    ...
    trs_HMap *m = nullptr;
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, TRS_HMAP_NEW_CAP(int32_t, int32_t, 16, trs_hash_i32, trs_eq_i32, &al, &m));
...
static void test_insert_reports_an_exhausted_arena_and_changes_nothing() {
    ...
    trs_HMap *m = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, trs_hash_i32, trs_eq_i32, arena, &m));
    put(m, 1, 10);
    trs_test_arena_leave(arena, 0);

    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, TRS_HMAP_INSERT(int32_t, int32_t, m, 2, 20, nullptr));
...
static void test_an_overwrite_needs_no_allocator() {
    ...
    TRS_TEST_OK(TRS_HMAP_INSERT(int32_t, int32_t, m, 1, 99, nullptr));
    assert_has(m, 1, 99);
...
static void test_reserve_reports_an_exhausted_arena() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_hmap_reserve(m, 100000));
...
static void test_copy_reports_an_exhausted_arena() {
    ...
    trs_HMap *src = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, int32_t, trs_hash_i32, trs_eq_i32, arena, &src));
    put(src, 1, 10);
    trs_test_arena_leave(arena, 0);

    trs_HMap *dst = nullptr;
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_hmap_copy(src, &dst));
    TEST_ASSERT_NULL(dst);
...
static void test_copy_assign_leaves_the_target_untouched_on_failure() {
    ...
    trs_HMap *src = make_filled(trs_hash_i32, 50);
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_hmap_copy_assign(src, dst));
...
static void test_get_or_insert_reports_an_exhausted_arena() {
    ...
    trs_HMapNode *node = nullptr;
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, TRS_HMAP_GET_OR_INSERT(int32_t, int32_t, m, 2, 20, &node));
...
static void test_eq_ignores_insertion_order_and_bucket_count() {
    trs_HMap *a = make_filled(trs_hash_i32, 40);

    trs_HMap *b = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW_CAP(int32_t, int32_t, 256, trs_hash_i32, trs_eq_i32, trs_al_default(), &b));
    for (int32_t i = 39; i >= 0; --i) {
        put(b, i, i * 10);
    }

    TEST_ASSERT_TRUE(trs_hmap_bucket_count(a) != trs_hmap_bucket_count(b));
    TEST_ASSERT_TRUE(trs_hmap_eq(a, a));
    TEST_ASSERT_TRUE(trs_hmap_eq(a, b));
    TEST_ASSERT_TRUE(trs_hmap_eq(b, a));
...
static void test_eq_looks_the_keys_up_with_the_hasher_of_the_other() {
    trs_HMap *a = make_filled(trs_hash_i32, 12);
    trs_HMap *b = make_filled(hash_all_alike, 12);
...
static void test_eq_parts_a_differing_value() {
    trs_HMap *a = make_filled(trs_hash_i32, 8);
    trs_HMap *b = make_filled(trs_hash_i32, 8);
    put(b, 3, -30);

    TEST_ASSERT_EQUAL_size_t(trs_hmap_len(a), trs_hmap_len(b));
    TEST_ASSERT_FALSE(trs_hmap_eq(a, b));
    TEST_ASSERT_FALSE(trs_hmap_eq(b, a));

    // ... unless the equality the caller names forgives the difference
    TEST_ASSERT_TRUE(trs_hmap_eq_by(a, b, eq_i32_abs));
    TEST_ASSERT_TRUE(trs_hmap_eq_by(b, a, eq_i32_abs));
...
static void test_eq_parts_a_differing_key() {
    trs_HMap *a = make_filled(trs_hash_i32, 8);
    trs_HMap *b = make_filled(trs_hash_i32, 8);
    TEST_ASSERT_TRUE(TRS_HMAP_REMOVE(int32_t, b, 3));
    put(b, 100, 30);
...
static void test_eq_parts_different_lengths() {
    trs_HMap *a = make_filled(trs_hash_i32, 8);
    trs_HMap *smaller = make_filled(trs_hash_i32, 7);

    TEST_ASSERT_FALSE(trs_hmap_eq(a, smaller));
    TEST_ASSERT_FALSE(trs_hmap_eq(smaller, a));

    trs_hmap_drop(a);
    trs_hmap_drop(smaller);
}

static void test_eq_of_two_empties() {
    trs_HMap *a = make_map(trs_hash_i32);
    trs_HMap *b = make_filled(trs_hash_i32, 8);
    trs_HMap *one = make_filled(trs_hash_i32, 1);

    trs_hmap_clear(b);

    TEST_ASSERT_TRUE(trs_hmap_eq(a, b));
    TEST_ASSERT_TRUE(trs_hmap_eq(b, a));
    TEST_ASSERT_FALSE(trs_hmap_eq(a, one));
...
static void test_eq_walks_whole_chains() {
    trs_HMap *a = make_filled(hash_all_alike, 16);
    trs_HMap *b = make_filled(hash_all_alike, 16);
    put(b, 15, -150);
...
static void test_eq_by_asks_the_equality_for_the_value_side() {
    trs_HMap *a = nullptr;
    trs_HMap *b = nullptr;
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, Pair, trs_hash_i32, trs_eq_i32, trs_al_default(), &a));
    TRS_TEST_OK(TRS_HMAP_NEW(int32_t, Pair, trs_hash_i32, trs_eq_i32, trs_al_default(), &b));

    for (int32_t i = 0; i < 8; ++i) {
        TRS_TEST_OK(trs_hmap_insert(a, &i, &(Pair){i, 10}, nullptr));
        TRS_TEST_OK(trs_hmap_insert(b, &i, &(Pair){i, 70}, nullptr));
    }

    TEST_ASSERT_FALSE(trs_hmap_eq(a, b));
    TEST_ASSERT_TRUE(trs_hmap_eq_by(a, b, trs_test_pair_eq_a));
...
static void test_eq_and_eq_by_agree_on_plain_values() {
    trs_HMap *a = make_filled(trs_hash_i32, 16);
    trs_HMap *b = make_filled(trs_hash_i32, 16);
    trs_HMap *smaller = make_filled(trs_hash_i32, 15);

    TEST_ASSERT_EQUAL(trs_hmap_eq(a, b), trs_hmap_eq_by(a, b, trs_eq_i32));
    TEST_ASSERT_EQUAL(trs_hmap_eq(a, smaller), trs_hmap_eq_by(a, smaller, trs_eq_i32));
...
static void test_eq_matches_a_copy() {
    trs_HMap *a = make_filled(trs_hash_i32, 24);

    trs_HMap *copy = nullptr;
    TRS_TEST_OK(trs_hmap_copy(a, &copy));

    TEST_ASSERT_TRUE(trs_hmap_eq(a, copy));
...
static void assert_prints(const char *expected, const trs_HMap *m) {
    FILE *stream = tmpfile();
    TEST_ASSERT_NOT_NULL(stream);

    trs_hmap_fprint(m, stream, trs_fprint_i32, trs_fprint_i32);
    rewind(stream);
...
static void test_print_writes_to_stdout() {
    ...
    trs_hmap_print(m, trs_fprint_i32, trs_fprint_i32);
