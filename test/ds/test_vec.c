...
static trs_Vec *make_vec(size_t len) {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_LEN(int32_t, len, trs_al_default(), &v));

    for (size_t i = 0; i < len; ++i) {
        TRS_VEC_SET(int32_t, v, i, (int32_t) i);
    }
    return v;
}

// empty int32_t vec with room for cap elems
static trs_Vec *make_vec_cap(size_t cap) {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_CAP(int32_t, cap, trs_al_default(), &v));

    return v;
}

static void push_int(trs_Vec *v, int32_t val) {
    TRS_TEST_OK(trs_vec_push(v, &val));
}

static void assert_elems(const trs_Vec *v, const int32_t *want, size_t n) {
    TEST_ASSERT_EQUAL_size_t(n, trs_vec_len(v));
...
static void test_new_starts_empty_and_unallocated() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW(int32_t, trs_al_default(), &v));

    TEST_ASSERT_EQUAL_size_t(0, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(0, trs_vec_cap(v));
    TEST_ASSERT_EQUAL_size_t(sizeof(int32_t), trs_vec_elem_size(v));
    TEST_ASSERT_EQUAL_PTR(trs_al_default(), trs_vec_al(v));
    TEST_ASSERT_NULL(trs_vec_data(v));
...
static void test_new_len_sets_len_cap_and_zeroes() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_LEN(int32_t, 4, trs_al_default(), &v));

    TEST_ASSERT_EQUAL_size_t(4, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(4, trs_vec_cap(v));

    constexpr int32_t zeroes[4] = {0, 0, 0, 0};
    TEST_ASSERT_EQUAL_INT32_ARRAY(zeroes, trs_vec_data(v), 4);
...
static void test_new_cap_reserves_without_length() {
    trs_Vec *v = make_vec_cap(8);

    TEST_ASSERT_EQUAL_size_t(0, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(8, trs_vec_cap(v));
    TEST_ASSERT_NOT_NULL(trs_vec_data(v));

    trs_vec_drop(v);
}

static void test_new_cap_zero_has_no_buffer() {
    trs_Vec *v = make_vec_cap(0);
...
static void test_from_data_copies_and_detaches_the_source() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_FROM_DATA(int32_t, src, 3, trs_al_default(), &v));

    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(3, trs_vec_cap(v));
    TEST_ASSERT_EQUAL_INT32_ARRAY(src, trs_vec_data(v), 3);
    TEST_ASSERT_TRUE((const void *) src != trs_vec_data(v));

    src[0] = 999;
    TEST_ASSERT_EQUAL_INT32(5, *TRS_VEC_GET_AS(int32_t, v, 0));
...
static void test_from_data_empty_has_no_buffer() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(trs_vec_from_data(nullptr, 0, sizeof(int32_t), trs_al_default(), &v));
...
static void test_from_data_copies_whole_elems() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_FROM_DATA(Pair, src, 2, trs_al_default(), &v));

    TEST_ASSERT_EQUAL_size_t(sizeof(Pair), trs_vec_elem_size(v));
    TEST_ASSERT_EQUAL_INT64(1, TRS_VEC_GET_AS(Pair, v, 0)->a);
    TEST_ASSERT_EQUAL_INT64(2, TRS_VEC_GET_AS(Pair, v, 0)->b);
    TEST_ASSERT_EQUAL_INT64(3, TRS_VEC_GET_AS(Pair, v, 1)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_VEC_GET_AS(Pair, v, 1)->b);
...
static void test_from_span_copies_the_view() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(trs_vec_from_span(TRS_SPAN_FROM_DATA(int32_t, src, 3), trs_al_default(), &v));
...
static void test_drop_null_is_noop() {
    trs_vec_drop(nullptr);
}

/* ========== copy ========== */

static void test_copy_is_independent() {
    trs_Vec *src = make_vec(4);

    trs_Vec *dst = nullptr;
    TRS_TEST_OK(trs_vec_copy(src, &dst));

    TEST_ASSERT_EQUAL_size_t(4, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_INT32_ARRAY(trs_vec_data(src), trs_vec_data(dst), 4);
    TEST_ASSERT_TRUE(trs_vec_data(src) != trs_vec_data(dst));

    TRS_VEC_SET(int32_t, src, 0, 999);
    TEST_ASSERT_EQUAL_INT32(0, *TRS_VEC_GET_AS(int32_t, dst, 0));
...
static void test_copy_fits_the_len_not_the_source_capacity() {
    ...
    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_size_t(2, trs_vec_cap(dst));
...
static void test_copy_of_empty_has_no_buffer() {
    trs_Vec *src = make_vec(0);

    trs_Vec *dst = nullptr;
    TRS_TEST_OK(trs_vec_copy(src, &dst));

    TEST_ASSERT_EQUAL_size_t(0, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_size_t(sizeof(int32_t), trs_vec_elem_size(dst));
    TEST_ASSERT_NULL(trs_vec_data(dst));
...
static void test_copy_inherits_the_source_allocator() {
    ...
    trs_Vec *src = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int32_t, arena, &src, 1, 2, 3));

    trs_Vec *dst = nullptr;
    TRS_TEST_OK(trs_vec_copy(src, &dst));

    TEST_ASSERT_EQUAL_PTR(arena, trs_vec_al(dst));
    TEST_ASSERT_EQUAL_INT32_ARRAY(trs_vec_data(src), trs_vec_data(dst), 3);
...
static void test_copy_with_builds_on_the_given_allocator() {
    ...
    trs_Vec *dst = nullptr;
    TRS_TEST_OK(trs_vec_copy_with(src, arena, &dst));

    TEST_ASSERT_EQUAL_PTR(arena, trs_vec_al(dst));
    TEST_ASSERT_EQUAL_PTR(trs_al_default(), trs_vec_al(src));
    TEST_ASSERT_TRUE(trs_vec_eq(src, dst));

    // the source is gone and the copy still holds the elems: they were taken, not viewed
    trs_vec_drop(src);
    TEST_ASSERT_EQUAL_INT32(3, *TRS_VEC_GET_AS(int32_t, dst, 3));
...
static void test_copy_with_reports_an_exhausted_target_arena() {
    ...
    trs_Vec *dst = nullptr;
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_copy_with(src, arena, &dst));
    TEST_ASSERT_NULL(dst);
    TEST_ASSERT_EQUAL_size_t(4, trs_vec_len(src));
...
static void test_move_assign_hands_over_the_contents_on_one_allocator() {
    ...
    trs_Vec *src = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int32_t, &al, &src, 1, 2, 3));

    trs_Vec *dst = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int32_t, &al, &dst, 9));

    const size_t requests = trs_test_probe_requests(&probe);
    TRS_TEST_OK(trs_vec_move_assign(src, dst));
    ...
    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_INT32(2, *TRS_VEC_GET_AS(int32_t, dst, 1));

    // the source is left empty and usable, its block handed back
    TEST_ASSERT_EQUAL_size_t(0, trs_vec_len(src));
    TEST_ASSERT_EQUAL_size_t(0, trs_vec_cap(src));
    TEST_ASSERT_NULL(trs_vec_data(src));
...
static void test_move_assign_across_allocators_empties_the_source() {
    ...
    trs_Vec *dst = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int32_t, arena, &dst, 9));
...
static void test_move_assign_across_allocators_reports_an_exhausted_arena() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_move_assign(src, dst));

    TEST_ASSERT_EQUAL_size_t(4, trs_vec_len(src));
    TEST_ASSERT_EQUAL_size_t(1, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_INT32(9, *TRS_VEC_GET_AS(int32_t, dst, 0));
...
static void test_move_assign_of_itself_changes_nothing() {
    trs_Vec *v = make_vec(3);

    TRS_TEST_OK(trs_vec_move_assign(v, v));

    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT32(2, *TRS_VEC_GET_AS(int32_t, v, 2));

    trs_vec_drop(v);
}

static void test_copy_assign_grows_and_shrinks_the_len() {
    trs_Vec *src = make_vec(6);
    trs_Vec *dst = make_vec(2);

    // grow: 2 -> 6
    TRS_TEST_OK(trs_vec_copy_assign(src, dst));
    TEST_ASSERT_EQUAL_size_t(6, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_INT32_ARRAY(trs_vec_data(src), trs_vec_data(dst), 6);

    // shrink: 6 -> 3
    trs_Vec *small = make_vec(3);
    TRS_TEST_OK(trs_vec_copy_assign(small, dst));
    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_INT32_ARRAY(trs_vec_data(small), trs_vec_data(dst), 3);

    // shrink to empty: the len goes, the buffer stays
    trs_Vec *empty = make_vec(0);
    TRS_TEST_OK(trs_vec_copy_assign(empty, dst));
    TEST_ASSERT_EQUAL_size_t(0, trs_vec_len(dst));
    TEST_ASSERT_NOT_NULL(trs_vec_data(dst));
...
static void test_copy_assign_keeps_the_target_capacity() {
    trs_Vec *src = make_vec(2);
    trs_Vec *dst = make_vec(6);

    TRS_TEST_OK(trs_vec_copy_assign(src, dst));

    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(dst));
    TEST_ASSERT_EQUAL_size_t(6, trs_vec_cap(dst));
...
static void test_copy_assign_self_is_noop() {
    ...
    TRS_TEST_OK(trs_vec_copy_assign(v, v));
    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT32(2, *TRS_VEC_GET_AS(int32_t, v, 2));
...
static void test_copy_assign_keeps_the_target_allocator() {
    ...
    trs_Vec *dst = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_LEN(int32_t, 1, arena, &dst));
...
static void test_bytes_is_len_times_elem_size() {
    trs_Vec *v = make_vec(4);

    TEST_ASSERT_EQUAL_size_t(4 * sizeof(int32_t), trs_vec_bytes(v));
...
static void test_bytes_ignores_the_spare_capacity() {
    ...
    TEST_ASSERT_EQUAL_size_t(8, trs_vec_cap(v));
    TEST_ASSERT_EQUAL_size_t(3 * sizeof(int32_t), trs_vec_bytes(v));
...
static void test_bytes_of_empty_is_zero() {
    ...
    TEST_ASSERT_EQUAL_size_t(0, trs_vec_bytes(v));
...
static void test_bytes_tracks_elem_size() {
    ...
    TEST_ASSERT_EQUAL_size_t(2 * sizeof(Pair), trs_vec_bytes(v));
...
static void test_bytes_agrees_with_the_span() {
    ...
    TEST_ASSERT_EQUAL_size_t(trs_span_bytes(trs_vec_to_span(v)), trs_vec_bytes(v));
...
static void test_set_get_roundtrip() {
    trs_Vec *v = make_vec(5);

    for (size_t i = 0; i < 5; ++i) {
        TEST_ASSERT_EQUAL_INT32((int32_t) i, *TRS_VEC_GET_AS(int32_t, v, i));
    }
...
static void test_get_mut_writes_through() {
    ...
    *TRS_VEC_GET_MUT_AS(int32_t, v, 1) = 42;
    TEST_ASSERT_EQUAL_INT32(42, *TRS_VEC_GET_AS(int32_t, v, 1));
...
static void test_first_and_last_address_the_ends() {
    ...
    TEST_ASSERT_EQUAL_INT32(0, *TRS_VEC_FRONT_AS(int32_t, v));
    TEST_ASSERT_EQUAL_INT32(3, *TRS_VEC_BACK_AS(int32_t, v));
    TEST_ASSERT_EQUAL_PTR(trs_vec_data(v), trs_vec_front(v));
    TEST_ASSERT_EQUAL_PTR(TRS_VEC_GET_AS(int32_t, v, 3), trs_vec_back(v));
...
static void test_first_and_last_mut_write_through() {
    ...
    *TRS_VEC_FRONT_MUT_AS(int32_t, v) = 10;
    *TRS_VEC_BACK_MUT_AS(int32_t, v) = 20;

    TEST_ASSERT_EQUAL_INT32(10, *TRS_VEC_GET_AS(int32_t, v, 0));
    TEST_ASSERT_EQUAL_INT32(20, *TRS_VEC_GET_AS(int32_t, v, 3));

    trs_vec_drop(v);
}

static void test_first_and_last_coincide_on_a_single_elem() {
    trs_Vec *v = make_vec(1);

    TEST_ASSERT_EQUAL_PTR(trs_vec_front(v), trs_vec_back(v));
    TEST_ASSERT_EQUAL_PTR(trs_vec_front_mut(v), trs_vec_back_mut(v));
...
static void test_last_follows_the_len() {
    ...
    TEST_ASSERT_EQUAL_INT32(2, *TRS_VEC_BACK_AS(int32_t, v));
    TEST_ASSERT_EQUAL_PTR(TRS_VEC_GET_AS(int32_t, v, 1), trs_vec_back(v));
...
static void test_data_mut_writes_through() {
    ...
    int32_t *d = trs_vec_data_mut(v);
    d[2] = 99;

    TEST_ASSERT_EQUAL_INT32(99, *TRS_VEC_GET_AS(int32_t, v, 2));
...
static void test_push_appends_and_doubles_the_capacity() {
    ...
    for (size_t i = 0; i < 5; ++i) {
        ...
        TEST_ASSERT_EQUAL_size_t(i + 1, trs_vec_len(v));
        TEST_ASSERT_EQUAL_size_t(want_cap[i], trs_vec_cap(v));
    }

    constexpr int32_t want[5] = {0, 1, 2, 3, 4};
    TEST_ASSERT_EQUAL_INT32_ARRAY(want, trs_vec_data(v), 5);
...
static void test_push_uses_reserved_capacity_without_reallocating() {
    trs_Vec *v = make_vec_cap(4);
    const void *before = trs_vec_data(v);
...
static void test_push_moves_whole_elems() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW(Pair, trs_al_default(), &v));

    TRS_TEST_OK(trs_vec_push(v, &(Pair){1, 2}));
    TRS_TEST_OK(trs_vec_push(v, &(Pair){3, 4}));
...
static void test_pop_shortens_and_keeps_the_capacity() {
    trs_Vec *v = make_vec(3);
    const size_t cap = trs_vec_cap(v);

    trs_vec_pop(v);

    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(cap, trs_vec_cap(v));
    TEST_ASSERT_EQUAL_INT32(1, *TRS_VEC_BACK_AS(int32_t, v));
...
static void test_push_after_pop_overwrites_the_slot() {
    ...
    TEST_ASSERT_EQUAL_PTR(before, trs_vec_data(v));
    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT32(77, *TRS_VEC_GET_AS(int32_t, v, 2));
...
static void test_insert_at_front_middle_and_end() {
    trs_Vec *v = make_vec(3); // 0, 1, 2

    TRS_TEST_OK(TRS_VEC_INSERT(int32_t, v, 0, 10));
    TRS_TEST_OK(TRS_VEC_INSERT(int32_t, v, 2, 20));
    TRS_TEST_OK(TRS_VEC_INSERT(int32_t, v, 5, 30));

    constexpr int32_t want[6] = {10, 0, 20, 1, 2, 30};
    TEST_ASSERT_EQUAL_size_t(6, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT32_ARRAY(want, trs_vec_data(v), 6);
...
static void test_insert_into_an_empty_vec() {
    ...
    TRS_TEST_OK(TRS_VEC_INSERT(int32_t, v, 0, 42));

    TEST_ASSERT_EQUAL_size_t(1, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(1, trs_vec_cap(v));
    TEST_ASSERT_EQUAL_INT32(42, *TRS_VEC_GET_AS(int32_t, v, 0));
...
static void test_insert_grows_when_full() {
    trs_Vec *v = make_vec(2); // len == cap == 2

    TRS_TEST_OK(TRS_VEC_INSERT(int32_t, v, 0, 9));

    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(v));
    TEST_ASSERT_TRUE(trs_vec_cap(v) >= 3);

    constexpr int32_t want[3] = {9, 0, 1};
    TEST_ASSERT_EQUAL_INT32_ARRAY(want, trs_vec_data(v), 3);

    trs_vec_drop(v);
}

static void test_remove_from_front_middle_and_end() {
    trs_Vec *v = make_vec(5); // 0, 1, 2, 3, 4

    trs_vec_remove(v, 0); // 1, 2, 3, 4
    trs_vec_remove(v, 1); // 1, 3, 4
    trs_vec_remove(v, 2); // 1, 3

    constexpr int32_t want[2] = {1, 3};
    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT32_ARRAY(want, trs_vec_data(v), 2);
...
static void test_insert_and_remove_move_whole_elems() {
    ...
    TRS_TEST_OK(trs_vec_insert(v, 1, &(Pair){5, 6}));

    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT64(5, TRS_VEC_GET_AS(Pair, v, 1)->a);
    TEST_ASSERT_EQUAL_INT64(6, TRS_VEC_GET_AS(Pair, v, 1)->b);
    TEST_ASSERT_EQUAL_INT64(3, TRS_VEC_GET_AS(Pair, v, 2)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_VEC_GET_AS(Pair, v, 2)->b);

    trs_vec_remove(v, 0);

    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT64(5, TRS_VEC_GET_AS(Pair, v, 0)->a);
    TEST_ASSERT_EQUAL_INT64(6, TRS_VEC_GET_AS(Pair, v, 0)->b);
    TEST_ASSERT_EQUAL_INT64(3, TRS_VEC_GET_AS(Pair, v, 1)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_VEC_GET_AS(Pair, v, 1)->b);
...
static void test_extend_appends_in_order() {
    ...
    TRS_TEST_OK(trs_vec_extend(v, TRS_SPAN_FROM_DATA(int32_t, src, 3)));
...
static void test_extend_onto_an_empty_vec() {
    ...
    TRS_TEST_OK(trs_vec_extend(v, TRS_SPAN_FROM_DATA(int32_t, src, 2)));
...
static void test_extend_with_an_empty_span_changes_nothing() {
    ...
    TRS_TEST_OK(trs_vec_extend(v, TRS_SPAN_FROM_DATA(int32_t, nullptr, 0)));
...
static void test_extend_takes_the_room_once() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW(int32_t, &al, &v));

    const size_t before = trs_test_probe_requests(&probe);

    constexpr int32_t src[8] = {1, 2, 3, 4, 5, 6, 7, 8};
    TRS_TEST_OK(trs_vec_extend(v, TRS_SPAN_FROM_DATA(int32_t, src, 8)));

    TEST_ASSERT_EQUAL_size_t(1, trs_test_probe_requests(&probe) - before);
...
static void test_a_run_of_extends_stays_amortized() {
    ...
    for (int32_t i = 0; i < 64; ++i) {
        TRS_TEST_OK(trs_vec_extend(v, TRS_SPAN_FROM_DATA(int32_t, &i, 1)));
    }

    TEST_ASSERT_EQUAL_size_t(64, trs_vec_len(v));
    TEST_ASSERT_TRUE(trs_test_probe_requests(&probe) < 20);
    TEST_ASSERT_EQUAL_INT32(63, *TRS_VEC_GET_AS(int32_t, v, 63));
...
static void test_extend_moves_wide_elems_whole() {
    ...
    TRS_TEST_OK(trs_vec_extend(v, TRS_SPAN_FROM_DATA(Pair, src, 2)));

    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT64(10, TRS_VEC_GET_AS(Pair, v, 0)->b);
    TEST_ASSERT_EQUAL_INT64(2, TRS_VEC_GET_AS(Pair, v, 1)->a);

    trs_vec_drop(v);
}

static void test_insert_span_puts_the_run_before_the_index() {
    trs_Vec *v = make_vec(4); // 0, 1, 2, 3
    constexpr int32_t src[2] = {8, 9};

    TRS_TEST_OK(trs_vec_insert_span(v, 1, TRS_SPAN_FROM_DATA(int32_t, src, 2)));
...
static void test_insert_span_at_the_front() {
    ...
    TRS_TEST_OK(trs_vec_insert_span(v, 0, TRS_SPAN_FROM_DATA(int32_t, src, 2)));
...
static void test_insert_span_at_len_is_extend() {
    ...
    TRS_TEST_OK(trs_vec_insert_span(v, trs_vec_len(v), TRS_SPAN_FROM_DATA(int32_t, src, 2)));
...
static void test_insert_span_of_an_empty_span_changes_nothing() {
    ...
    TRS_TEST_OK(trs_vec_insert_span(v, 1, TRS_SPAN_FROM_DATA(int32_t, nullptr, 0)));
...
static void test_insert_span_keeps_a_long_tail_in_order() {
    trs_Vec *v = make_vec(16);
    constexpr int32_t src[2] = {100, 200};

    TRS_TEST_OK(trs_vec_insert_span(v, 2, TRS_SPAN_FROM_DATA(int32_t, src, 2)));

    TEST_ASSERT_EQUAL_size_t(18, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT32(1, *TRS_VEC_GET_AS(int32_t, v, 1));
    TEST_ASSERT_EQUAL_INT32(100, *TRS_VEC_GET_AS(int32_t, v, 2));
    TEST_ASSERT_EQUAL_INT32(200, *TRS_VEC_GET_AS(int32_t, v, 3));
    for (size_t i = 4; i < 18; ++i) {
        TEST_ASSERT_EQUAL_INT32((int32_t) i - 2, *TRS_VEC_GET_AS(int32_t, v, i));
    }
...
static void test_remove_range_closes_the_gap() {
    trs_Vec *v = make_vec(6); // 0 .. 5

    trs_vec_remove_range(v, 1, 3);
...
static void test_remove_range_moves_wide_elems_whole() {
    ...
    TRS_TEST_OK(trs_vec_extend(v, TRS_SPAN_FROM_DATA(Pair, src, 4)));

    trs_vec_remove_range(v, 1, 2);

    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT64(1, TRS_VEC_GET_AS(Pair, v, 0)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_VEC_GET_AS(Pair, v, 1)->a);
    TEST_ASSERT_EQUAL_INT64(40, TRS_VEC_GET_AS(Pair, v, 1)->b);
...
static void test_clear_drops_the_len_and_keeps_the_buffer() {
    ...
    push_int(v, 7);
    TEST_ASSERT_EQUAL_PTR(before, trs_vec_data(v));
    TEST_ASSERT_EQUAL_INT32(7, *TRS_VEC_GET_AS(int32_t, v, 0));

    trs_vec_drop(v);
}

static void test_clear_of_an_empty_vec_is_a_noop() {
    trs_Vec *v = make_vec(0);
...
static void test_reserve_grows_the_capacity_and_keeps_the_contents() {
    ...
    TRS_TEST_OK(trs_vec_reserve(v, 16));

    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(16, trs_vec_cap(v));
...
static void test_reserve_below_the_capacity_is_a_noop() {
    ...
    TRS_TEST_OK(trs_vec_reserve(v, 2));
    TRS_TEST_OK(trs_vec_reserve(v, 8));
    TRS_TEST_OK(trs_vec_reserve(v, 0));
...
static void test_reserve_reports_size_overflow() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_reserve(v, SIZE_MAX));
...
static void test_shrink_to_fit_drops_the_slack() {
    trs_Vec *v = make_vec_cap(16);
    push_int(v, 1);
    push_int(v, 2);
    push_int(v, 3);

    TRS_TEST_OK(trs_vec_shrink_to_fit(v));
...
static void test_resize_up_zeroes_the_tail() {
    trs_Vec *v = make_vec(2); // 0, 1

    TRS_TEST_OK(trs_vec_resize(v, 5));

    constexpr int32_t want[5] = {0, 1, 0, 0, 0};
    TEST_ASSERT_EQUAL_size_t(5, trs_vec_len(v));
    TEST_ASSERT_EQUAL_INT32_ARRAY(want, trs_vec_data(v), 5);
...
static void test_resize_down_keeps_the_head_and_the_capacity() {
    ...
    TRS_TEST_OK(trs_vec_resize(v, 2));

    constexpr int32_t want[2] = {0, 1};
    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(5, trs_vec_cap(v));
    TEST_ASSERT_EQUAL_INT32_ARRAY(want, trs_vec_data(v), 2);
...
static void test_resize_to_zero_keeps_the_buffer() {
    ...
    TRS_TEST_OK(trs_vec_resize(v, 0));
...
static void test_resize_to_the_same_len_is_a_noop() {
    ...
    TRS_TEST_OK(trs_vec_resize(v, 3));
...
static void test_resize_reports_size_overflow() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_resize(v, SIZE_MAX));
...
static void test_swap_same_allocator_hands_over_buffers() {
    trs_Vec *a = make_vec(2);
    trs_Vec *b = make_vec(5);

    const void *pa = trs_vec_data(a);
    const void *pb = trs_vec_data(b);

    trs_vec_swap(a, b);

    TEST_ASSERT_EQUAL_PTR(pb, trs_vec_data(a));
    TEST_ASSERT_EQUAL_PTR(pa, trs_vec_data(b));
    TEST_ASSERT_EQUAL_size_t(5, trs_vec_len(a));
    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(b));
...
static void test_swap_same_allocator_carries_the_capacity() {
    trs_Vec *a = make_vec_cap(16);
    push_int(a, 1);

    trs_Vec *b = make_vec(3);

    trs_vec_swap(a, b);

    TEST_ASSERT_EQUAL_size_t(3, trs_vec_len(a));
    TEST_ASSERT_EQUAL_size_t(3, trs_vec_cap(a));
    TEST_ASSERT_EQUAL_size_t(1, trs_vec_len(b));
    TEST_ASSERT_EQUAL_size_t(16, trs_vec_cap(b));
...
static void test_swap_elems_exchanges_the_pair() {
    ...
    trs_vec_swap_elems(v, 0, 3);

    constexpr int32_t want[4] = {3, 1, 2, 0};
    TEST_ASSERT_EQUAL_INT32_ARRAY(want, trs_vec_data(v), 4);
...
static void test_swap_elems_moves_wide_elems_whole() {
    ...
    trs_vec_swap_elems(v, 0, 1);

    TEST_ASSERT_EQUAL_INT64(3, TRS_VEC_GET_AS(Pair, v, 0)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_VEC_GET_AS(Pair, v, 0)->b);
    TEST_ASSERT_EQUAL_INT64(1, TRS_VEC_GET_AS(Pair, v, 1)->a);
    TEST_ASSERT_EQUAL_INT64(2, TRS_VEC_GET_AS(Pair, v, 1)->b);
...
static void test_to_span_follows_the_len_not_the_capacity() {
    ...
    const trs_Span s = trs_vec_to_span(v);

    TEST_ASSERT_EQUAL_PTR(trs_vec_data(v), s.data);
    TEST_ASSERT_EQUAL_size_t(3, s.len);
    TEST_ASSERT_EQUAL_size_t(sizeof(int32_t), s.elem_size);
    TEST_ASSERT_EQUAL_INT32(3, *TRS_SPAN_GET_AS(int32_t, s, 2));
...
static void test_to_span_mut_writes_reach_the_vec() {
    ...
    const trs_SpanMut s = trs_vec_to_span_mut(v);
    TRS_SPAN_SET(int32_t, s, 0, 77);

    TEST_ASSERT_EQUAL_INT32(77, *TRS_VEC_GET_AS(int32_t, v, 0));
...
static void test_new_len_reports_size_overflow() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_new_len(SIZE_MAX, 2, trs_al_default(), &v));
...
static void test_new_cap_reports_size_overflow() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_new_cap(SIZE_MAX, 2, trs_al_default(), &v));
...
static void test_from_data_reports_size_overflow() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_from_data(src, SIZE_MAX, 2, trs_al_default(), &v));
...
static void test_new_len_reports_an_exhausted_arena() {
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, TRS_VEC_NEW_LEN(int32_t, 1000, arena, &v));
...
static void test_push_reports_an_exhausted_arena() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_CAP(int32_t, 2, arena, &v));
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, TRS_VEC_PUSH(int32_t, v, 3));

    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(2, trs_vec_cap(v));
    TEST_ASSERT_EQUAL_PTR(before, trs_vec_data(v));
...
static void test_push_falls_back_to_one_more_slot_when_doubling_fails() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_CAP(Pair, 2, arena, &v));

    TRS_TEST_OK(trs_vec_push(v, &(Pair){1, 2}));
    TRS_TEST_OK(trs_vec_push(v, &(Pair){3, 4}));

    // room for three elems, not for the four that doubling would ask for
    trs_test_arena_leave(arena, for_three);

    TRS_TEST_OK(trs_vec_push(v, &(Pair){5, 6}));
    ...
    TEST_ASSERT_EQUAL_INT64(1, TRS_VEC_GET_AS(Pair, v, 0)->a);
    TEST_ASSERT_EQUAL_INT64(3, TRS_VEC_GET_AS(Pair, v, 1)->a);
    TEST_ASSERT_EQUAL_INT64(5, TRS_VEC_GET_AS(Pair, v, 2)->a);
    TEST_ASSERT_EQUAL_INT64(6, TRS_VEC_GET_AS(Pair, v, 2)->b);
...
static void test_extend_reports_an_exhausted_arena_and_changes_nothing() {
    ...
    constexpr int32_t src[3] = {7, 8, 9};
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_extend(v, TRS_SPAN_FROM_DATA(int32_t, src, 3)));
...
static void test_insert_span_reports_an_exhausted_arena_and_changes_nothing() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_CAP(int32_t, 3, arena, &v));
    push_int(v, 1);
    push_int(v, 2);
    push_int(v, 3);
    ...
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_insert_span(v, 1, TRS_SPAN_FROM_DATA(int32_t, src, 2)));
...
static void test_extend_reports_size_overflow() {
    ...
    constexpr int32_t src[1] = {1};
    TRS_TEST_STATUS(TRS_STATUS_ERR_NO_MEM, trs_vec_extend(v, TRS_SPAN_FROM_DATA(int32_t, src, SIZE_MAX)));
...
static void test_extend_within_the_capacity_needs_no_allocator() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_NEW_CAP(int32_t, 8, arena, &v));
    push_int(v, 1);
...
static void test_macro_of_builds_from_literals() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int32_t, trs_al_default(), &v, 4, 5, 6));
...
static void test_macro_of_derives_len_from_the_list() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int64_t, trs_al_default(), &v, 1, 2, 3, 4, 5));

    TEST_ASSERT_EQUAL_size_t(5, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(sizeof(int64_t), trs_vec_elem_size(v));
...
static void test_macro_push_appends_a_value() {
    ...
    TRS_TEST_OK(TRS_VEC_PUSH(int32_t, v, 4));
    TRS_TEST_OK(TRS_VEC_PUSH(int32_t, v, 5));
...
static void test_macro_push_evaluates_its_value_once() {
    ...
    int32_t next = 0;
    for (int i = 0; i < 3; ++i) {
        TRS_TEST_OK(TRS_VEC_PUSH(int32_t, v, next++));
    }
...
static void test_macro_from_data_infers_elem_size() {
    ...
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_FROM_DATA(int32_t, src, 2, trs_al_default(), &v));

    TEST_ASSERT_EQUAL_size_t(2, trs_vec_len(v));
    TEST_ASSERT_EQUAL_size_t(sizeof(int32_t), trs_vec_elem_size(v));
    TEST_ASSERT_EQUAL_INT32_ARRAY(src, trs_vec_data(v), 2);
...
static void test_macro_extend_appends_a_literal_list() {
    ...
    TRS_TEST_OK(TRS_VEC_EXTEND(int32_t, v, 4, 5, 6));
...
static void test_macro_insert_span_places_a_literal_list() {
    ...
    TRS_TEST_OK(TRS_VEC_INSERT_SPAN(int32_t, v, 1, 8, 9));
...
static void test_eq_matches_the_same_elems() {
    trs_Vec *a = make_vec(4);
    trs_Vec *b = make_vec(4);

    TEST_ASSERT_TRUE(trs_vec_eq(a, a));
    TEST_ASSERT_TRUE(trs_vec_eq(a, b));
    TEST_ASSERT_TRUE(trs_vec_eq(b, a));
    TEST_ASSERT_TRUE(trs_vec_eq_by(a, b, trs_eq_i32));
...
static void test_eq_parts_one_differing_elem() {
    trs_Vec *a = make_vec(4);
    trs_Vec *b = make_vec(4);
    TRS_VEC_SET(int32_t, b, 3, 99);

    TEST_ASSERT_FALSE(trs_vec_eq(a, b));
    TEST_ASSERT_FALSE(trs_vec_eq_by(a, b, trs_eq_i32));
...
static void test_eq_parts_different_lengths() {
    trs_Vec *a = make_vec(4);
    trs_Vec *shorter = make_vec(3);

    TEST_ASSERT_FALSE(trs_vec_eq(a, shorter));
    TEST_ASSERT_FALSE(trs_vec_eq(shorter, a));

    trs_vec_drop(a);
    trs_vec_drop(shorter);
}

static void test_eq_of_two_empties() {
    trs_Vec *a = make_vec(0);
    trs_Vec *b = make_vec_cap(8);
    trs_Vec *one = make_vec(1);

    TEST_ASSERT_TRUE(trs_vec_eq(a, b));
    TEST_ASSERT_TRUE(trs_vec_eq_by(a, b, trs_eq_i32));
    TEST_ASSERT_FALSE(trs_vec_eq(a, one));
...
static void test_eq_ignores_capacity() {
    trs_Vec *a = make_vec(3);
    trs_Vec *b = make_vec_cap(64);
    for (int32_t i = 0; i < 3; ++i) {
        push_int(b, i);
    }

    TEST_ASSERT_TRUE(trs_vec_cap(a) != trs_vec_cap(b));
    TEST_ASSERT_TRUE(trs_vec_eq(a, b));
    TEST_ASSERT_TRUE(trs_vec_eq_by(a, b, trs_eq_i32));
...
static void test_eq_by_asks_the_equality() {
    ...
    trs_Vec *a = nullptr;
    trs_Vec *b = nullptr;
    TRS_TEST_OK(TRS_VEC_FROM_DATA(Pair, lhs, 2, trs_al_default(), &a));
    TRS_TEST_OK(TRS_VEC_FROM_DATA(Pair, rhs, 2, trs_al_default(), &b));

    TEST_ASSERT_FALSE(trs_vec_eq(a, b));
    TEST_ASSERT_TRUE(trs_vec_eq_by(a, b, trs_test_pair_eq_a));
...
static void assert_prints(const char *expected, const trs_Vec *v) {
    FILE *stream = tmpfile();
    TEST_ASSERT_NOT_NULL(stream);

    trs_vec_fprint(v, stream, trs_fprint_i32);
    rewind(stream);
...
static void test_fprint_writes_the_elems() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int32_t, trs_al_default(), &v, 5, 3, 1));
...
static void test_fprint_of_a_single_elem_has_no_separator() {
    trs_Vec *v = nullptr;
    TRS_TEST_OK(TRS_VEC_OF(int32_t, trs_al_default(), &v, 7));
...
static void test_print_writes_to_stdout() {
    ...
    trs_vec_print(v, trs_fprint_i32);
