...
// the ring is reused as it empties: pushing and popping in step keeps at most three elems
// alive while a hundred pass through, so the buffer wraps many times over
...
static void test_new_reports_an_exhausted_arena() {
    ...
    nad_Queue *q = nullptr;
    NAD_TEST_STATUS(NAD_STATUS_OUT_OF_MEMORY, NAD_QUEUE_NEW(int32_t, arena, &q));
    TEST_ASSERT_NULL(q);
...
static void test_push_reports_an_exhausted_arena_and_changes_nothing() {
    ...
    NAD_TEST_STATUS(NAD_STATUS_OUT_OF_MEMORY, nad_queue_push(q, &val));
...
static void test_copy_reports_an_exhausted_arena() {
    ...
    nad_Queue *dst = nullptr;
    NAD_TEST_STATUS(NAD_STATUS_OUT_OF_MEMORY, nad_queue_copy(src, &dst));
...
static void test_reserve_reports_an_exhausted_arena() {
    ...
    NAD_TEST_STATUS(NAD_STATUS_OUT_OF_MEMORY, nad_queue_reserve(q, 1000));
...
static void test_a_refused_header_frees_the_deque() {
    ...
    nad_Queue *q = nullptr;
    NAD_TEST_STATUS(NAD_STATUS_OUT_OF_MEMORY, NAD_QUEUE_NEW(int32_t, &al, &q));
