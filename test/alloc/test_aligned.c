...
static void test_new_takes_two_blocks_and_drop_returns_them() {
    ...
    trs_Al *al = trs_al_aligned_new(&parent, 64);
    TEST_ASSERT_NOT_NULL(al);
    TEST_ASSERT_EQUAL_size_t(2, probe.live);

    trs_al_aligned_drop(al);
...
static void test_new_cleans_up_after_a_failing_parent() {
    ...
    TEST_ASSERT_NULL(trs_al_aligned_new(&parent, 64));
    TEST_ASSERT_EQUAL_size_t(0, probe.live);
...
static void test_blocks_do_not_overlap() {
    trs_Al *al = trs_al_aligned_new(trs_al_default(), 64);
    TEST_ASSERT_NOT_NULL(al);

    unsigned char *a = trs_alloc(al, 100);
    unsigned char *b = trs_alloc(al, 100);
    ...
    trs_dealloc(al, a, 100);
    trs_dealloc(al, b, 100);
    trs_al_aligned_drop(al);
}
...
static void test_alloc_rejects_a_size_the_padding_would_overflow() {
    ...
    const size_t before = trs_test_probe_requests(&probe);
    TEST_ASSERT_NULL(trs_alloc(al, SIZE_MAX));
    TEST_ASSERT_NULL(trs_alloc(al, SIZE_MAX - sizeof(void *)));
    TEST_ASSERT_EQUAL_size_t(before, trs_test_probe_requests(&probe));
...
static void test_alloc_fails_when_the_parent_does() {
    ...
    trs_test_probe_fail_after_next(&probe, 0);
    TEST_ASSERT_NULL(trs_alloc(al, 100));
...
// no calloc of its own, so trs_calloc must build one on top of this alloc — never
...
static void test_calloc_zeroes_an_aligned_block_without_the_parents_calloc() {
    ...
    unsigned char *ptr = trs_calloc(al, 8, 16);
    TEST_ASSERT_NOT_NULL(ptr);
...
static void test_realloc_keeps_the_bytes_and_the_alignment() {
    ...
    unsigned char *ptr = trs_alloc(al, 100);
    TEST_ASSERT_NOT_NULL(ptr);
    memset(ptr, 0x5A, 100);

    unsigned char *grown = trs_realloc(al, ptr, 100, 500);
    ...
    unsigned char *shrunk = trs_realloc(al, grown, 500, 50);
...
static void test_realloc_to_zero_gives_the_block_back() {
    ...
    TEST_ASSERT_NULL(trs_realloc(al, ptr, 100, 0));
    TEST_ASSERT_EQUAL_size_t(2, probe.live);
...
static void test_over_an_arena() {
    ...
    trs_Al *al = trs_al_aligned_new(arena, 128);
    TEST_ASSERT_NOT_NULL(al);
...
static void test_over_itself() {
    trs_Al *inner = trs_al_aligned_new(trs_al_default(), 64);
    TEST_ASSERT_NOT_NULL(inner);

    trs_Al *outer = trs_al_aligned_new(inner, 256);
    TEST_ASSERT_NOT_NULL(outer);

    void *ptr = trs_alloc(outer, 300);
    TEST_ASSERT_NOT_NULL(ptr);
    TEST_ASSERT_EQUAL_size_t(0, (uintptr_t) ptr % 256);

    trs_dealloc(outer, ptr, 300);
    trs_al_aligned_drop(outer);
    trs_al_aligned_drop(inner);
}
