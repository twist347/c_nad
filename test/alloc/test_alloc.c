 ...
 * only alloc/dealloc, which forces trs_calloc/trs_realloc down their fallback
 * paths, and a "full" one whose own calloc/realloc must be preferred over those.
 */

static trs_TestProbe probe;

static trs_Al bare_al() {
    return trs_test_probe_bare(&probe);
}

static trs_Al full_al() {
    return trs_test_probe_full(&probe);
}
...
static void test_alloc_zero_is_null_and_does_not_dispatch() {
    ...
    TEST_ASSERT_NULL(trs_alloc(&al, 0));
...
static void test_alloc_propagates_failure() {
    ...
    TEST_ASSERT_NULL(trs_alloc(&al, 16));
    TEST_ASSERT_EQUAL_size_t(1, probe.alloc_calls);
}
...
static void test_calloc_prefers_the_native_hook() {
    ...
    unsigned char *p = trs_calloc(&al, 4, 8);
    TEST_ASSERT_NOT_NULL(p);
...
// without one, trs_calloc synthesizes it from alloc + memset
...
static void test_calloc_rejects_overflow_on_either_path() {
    trs_Al bare = bare_al();
    TEST_ASSERT_NULL(trs_calloc(&bare, SIZE_MAX, 2));
    TEST_ASSERT_EQUAL_size_t(0, probe.alloc_calls);

    trs_Al full = full_al();
    TEST_ASSERT_NULL(trs_calloc(&full, SIZE_MAX, 2));
    TEST_ASSERT_EQUAL_size_t(0, probe.calloc_calls);
}
...
static void test_calloc_fallback_propagates_failure() {
    ...
    TEST_ASSERT_NULL(trs_calloc(&al, 4, 8));
    TEST_ASSERT_EQUAL_size_t(1, probe.alloc_calls);
}
...
static void test_realloc_fallback_grows_and_keeps_the_contents() {
    ...
    unsigned char *q = trs_realloc(&al, p, 8, 32);
    TEST_ASSERT_NOT_NULL(q);
...
static void test_realloc_fallback_shrinks() {
    ...
    unsigned char *p = trs_alloc(&al, 16);
    for (size_t i = 0; i < 16; ++i) {
        p[i] = (unsigned char) (i + 1);
    }

    unsigned char *q = trs_realloc(&al, p, 16, 4);
    TEST_ASSERT_NOT_NULL(q);
...
static void test_realloc_fallback_failure_keeps_the_original() {
    ...
    TEST_ASSERT_NULL(trs_realloc(&al, p, 8, 64));
...
/* ========== macros ==========
 *
 * These are the only cases that expand TRS_ALLOC and friends. Nothing else in the
 ...
 *
 * TRS_ALLOC and TRS_REALLOC evaluate their count twice, so every count here is a
 * plain value.
 */
...
static void test_macro_alloc_scales_the_count_by_elem_size() {
    ...
    int32_t *p = TRS_ALLOC(int32_t, &al, 4);
    TEST_ASSERT_NOT_NULL(p);
...
static void test_macro_alloc_rejects_a_count_that_would_wrap() {
    ...
    TEST_ASSERT_NULL(TRS_ALLOC(int32_t, &al, SIZE_MAX / sizeof(int32_t) + 2));
    TEST_ASSERT_EQUAL_size_t(0, probe.alloc_calls);
}
...
static void test_macro_alloc_passes_the_largest_fitting_count_through() {
    ...
    TEST_ASSERT_NULL(TRS_ALLOC(int32_t, &al, SIZE_MAX / sizeof(int32_t)));
    TEST_ASSERT_EQUAL_size_t(1, probe.alloc_calls);
}
...
static void test_macro_calloc_hands_the_operands_over_unmultiplied() {
    ...
    int32_t *p = TRS_CALLOC(int32_t, &al, 4);
    TEST_ASSERT_NOT_NULL(p);
    TEST_ASSERT_EQUAL_size_t(1, probe.calloc_calls);
...
// the count is never multiplied by the macro, so the overflow is trs_calloc's to catch
// and TRS_CALLOC needs no guard of its own — see test_calloc_rejects_overflow_on_either_path
// for the contract this leans on.
static void test_macro_calloc_overflow_is_caught_below_the_macro() {
    trs_Al al = bare_al();

    TEST_ASSERT_NULL(TRS_CALLOC(int32_t, &al, SIZE_MAX));
    TEST_ASSERT_EQUAL_size_t(0, probe.alloc_calls);
}
...
static void test_macro_realloc_scales_both_counts() {
    ...
    int32_t *q = TRS_REALLOC(int32_t, &al, p, 4, 8);
    TEST_ASSERT_NOT_NULL(q);
    ...
    TRS_DEALLOC(int32_t, &al, q, 8);
}

// new_count * sizeof(T) wraps to exactly 0 here. Unguarded that reaches trs_realloc as
...
static void test_macro_realloc_rejects_a_new_count_that_would_wrap() {
    ...
    TEST_ASSERT_NULL(TRS_REALLOC(int32_t, &al, p, 4, (size_t) 1 << 62));
