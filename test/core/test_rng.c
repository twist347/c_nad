...
static void test_rng_matches_the_reference_stream() {
    ...
    trs_Rng rng = trs_rng_from_seed(0);
    for (size_t i = 0; i < sizeof seed_0 / sizeof *seed_0; ++i) {
        TEST_ASSERT_EQUAL_UINT64(seed_0[i], trs_rng_u64(&rng));
    }

    rng = trs_rng_from_seed(42);
    for (size_t i = 0; i < sizeof seed_42 / sizeof *seed_42; ++i) {
        TEST_ASSERT_EQUAL_UINT64(seed_42[i], trs_rng_u64(&rng));
    }
}
...
static void test_rng_replays_the_same_seed() {
    trs_Rng a = trs_rng_from_seed(12345);
    trs_Rng b = trs_rng_from_seed(12345);

    for (size_t i = 0; i < 1000; ++i) {
        TEST_ASSERT_EQUAL_UINT64(trs_rng_u64(&a), trs_rng_u64(&b));
    }
}
...
static void test_rng_takes_zero_as_a_seed() {
    trs_Rng zero = trs_rng_from_seed(0);
    trs_Rng one = trs_rng_from_seed(1);

    size_t agreements = 0;
    for (size_t i = 0; i < 100; ++i) {
        const uint64_t val = trs_rng_u64(&zero);
        TEST_ASSERT_NOT_EQUAL_UINT64(0, val);

        if (val == trs_rng_u64(&one)) {
            ++agreements;
        }
    }
...
static void test_rng_copies_as_a_value() {
    ...
    for (size_t i = 0; i < 10; ++i) {
        (void) trs_rng_u64(&rng);
    }

    trs_Rng fork = rng;
    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_UINT64(trs_rng_u64(&rng), trs_rng_u64(&fork));
    }
}
...
static void test_rng_u32_is_the_top_half_of_a_draw() {
    ...
    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_UINT32((uint32_t) (trs_rng_u64(&a) >> 32), trs_rng_u32(&b));
    }
}
...
static void test_rng_u64_max_stays_in_range() {
    ...
    for (size_t m = 0; m < sizeof maxes / sizeof *maxes; ++m) {
        for (size_t i = 0; i < 2000; ++i) {
            TEST_ASSERT_TRUE(trs_rng_u64_max(&rng, maxes[m]) <= maxes[m]);
        }
    }
}
...
static void test_rng_u64_max_takes_the_full_width() {
    trs_Rng bounded = trs_rng_from_seed(99);
    trs_Rng raw = trs_rng_from_seed(99);

    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_UINT64(trs_rng_u64(&raw), trs_rng_u64_max(&bounded, UINT64_MAX));
    }
}
...
static void test_rng_u32_max_stays_in_range() {
    ...
    for (size_t m = 0; m < sizeof maxes / sizeof *maxes; ++m) {
        for (size_t i = 0; i < 2000; ++i) {
            TEST_ASSERT_TRUE(trs_rng_u32_max(&rng, maxes[m]) <= maxes[m]);
        }
    }
}
...
static void test_rng_u64_max_is_flat_over_a_bound_that_does_not_divide_the_width() {
    trs_Rng rng = trs_rng_from_seed(4);

    size_t buckets[3] = {0};
    for (size_t i = 0; i < TRIALS; ++i) {
        ++buckets[trs_rng_u64_max(&rng, 2)];
    }
...
static void test_rng_u64_max_rejects_where_the_range_does_not_fit() {
    ...
    trs_Rng rng = trs_rng_from_seed(17);
    size_t even = 0;

    for (size_t i = 0; i < TRIALS; ++i) {
        even += (trs_rng_u64_max(&rng, max) & 1) == 0 ? 1 : 0;
    }
...
static void test_rng_idx_covers_every_position() {
    ...
    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_size_t(0, trs_rng_idx(&rng, 1));
    }

    size_t buckets[4] = {0};
    for (size_t i = 0; i < TRIALS; ++i) {
        const size_t idx = trs_rng_idx(&rng, 4);
...
static void test_rng_i32_range_holds_its_ends() {
    ...
    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_INT32(-7, trs_rng_i32_range(&rng, -7, -7));
    }

    bool saw_lo = false;
    bool saw_hi = false;
    for (size_t i = 0; i < 1000; ++i) {
        const int32_t val = trs_rng_i32_range(&rng, -2, 2);
...
static void test_rng_i32_range_survives_the_whole_width() {
    ...
    for (size_t i = 0; i < 1000; ++i) {
        const int32_t val = trs_rng_i32_range(&rng, INT32_MIN, INT32_MAX);
...
static void test_rng_i64_range_survives_the_whole_width() {
    trs_Rng rng = trs_rng_from_seed(9);

    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_INT64(42, trs_rng_i64_range(&rng, 42, 42));
    }
    ...
    for (size_t i = 0; i < 1000; ++i) {
        const int64_t val = trs_rng_i64_range(&rng, INT64_MIN, INT64_MAX);
        saw_negative = saw_negative || val < 0;
        saw_positive = saw_positive || val > 0;
    }
...
static void test_rng_f64_stays_below_one() {
    trs_Rng rng = trs_rng_from_seed(10);

    double lowest = 1.0;
    double highest = 0.0;

    for (size_t i = 0; i < TRIALS; ++i) {
        const double val = trs_rng_f64(&rng);
...
static void test_rng_f32_stays_below_one() {
    trs_Rng rng = trs_rng_from_seed(11);

    for (size_t i = 0; i < TRIALS; ++i) {
        const float val = trs_rng_f32(&rng);
        TEST_ASSERT_TRUE(val >= 0.0f && val < 1.0f);
    }
}

static void test_rng_f64_spreads_evenly() {
    trs_Rng rng = trs_rng_from_seed(12);

    size_t buckets[10] = {0};
    for (size_t i = 0; i < TRIALS; ++i) {
        ++buckets[(size_t) (trs_rng_f64(&rng) * 10.0)];
    }
...
static void test_rng_f64_range_stays_half_open() {
    trs_Rng rng = trs_rng_from_seed(13);

    for (size_t i = 0; i < TRIALS; ++i) {
        const double val = trs_rng_f64_range(&rng, -2.5, 7.5);
        TEST_ASSERT_TRUE(val >= -2.5 && val < 7.5);
    }

    // a width no int could hold, and one narrow enough that the scaling rounds
    for (size_t i = 0; i < 1000; ++i) {
        const double wide = trs_rng_f64_range(&rng, -1e300, 1e300);
        TEST_ASSERT_TRUE(wide >= -1e300 && wide < 1e300);

        const double narrow = trs_rng_f64_range(&rng, 1.0, nextafter(1.0, 2.0));
...
static void test_rng_f64_range_collapses_when_the_ends_meet() {
    trs_Rng rng = trs_rng_from_seed(14);

    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_DOUBLE(3.5, trs_rng_f64_range(&rng, 3.5, 3.5));
    }
}

static void test_rng_f32_range_stays_half_open() {
    trs_Rng rng = trs_rng_from_seed(15);

    for (size_t i = 0; i < TRIALS; ++i) {
        const float val = trs_rng_f32_range(&rng, -1.0f, 1.0f);
        TEST_ASSERT_TRUE(val >= -1.0f && val < 1.0f);
    }

    for (size_t i = 0; i < 100; ++i) {
        TEST_ASSERT_EQUAL_FLOAT(0.25f, trs_rng_f32_range(&rng, 0.25f, 0.25f));
    }
}
...
static void test_rng_bool_is_a_fair_coin() {
    trs_Rng rng = trs_rng_from_seed(16);
    ...
    for (size_t i = 0; i < TRIALS; ++i) {
        const bool val = trs_rng_bool(&rng);
        heads += val ? 1 : 0;
