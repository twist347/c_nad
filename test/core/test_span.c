...
static void assert_prints(const char *expected, trs_Span s) {
    FILE *stream = tmpfile();
    TEST_ASSERT_NOT_NULL(stream);

    trs_span_fprint(s, stream, trs_fprint_i32);
    rewind(stream);
...
static void assert_mut_prints(const char *expected, trs_SpanMut s) {
    FILE *stream = tmpfile();
    TEST_ASSERT_NOT_NULL(stream);

    trs_span_mut_fprint(s, stream, trs_fprint_i32);
    rewind(stream);
...
static void test_for_each_binds_every_elem_in_order() {
    ...
    int32_t seen[4];
    size_t n = 0;
    TRS_SPAN_FOR_EACH_AS(int32_t, elem, s) {
        seen[n++] = *elem;
    }
...
// the view is taken once: a walk over trs_span_sub of a call would otherwise redo it
static void test_for_each_evaluates_the_view_once() {
    constexpr int32_t buf[4] = {1, 2, 3, 4};

    int32_t total = 0;
    TRS_SPAN_FOR_EACH_AS(int32_t, elem, trs_span_sub(TRS_SPAN_FROM_DATA(int32_t, buf, 4), 1, 3)) {
        total += *elem;
    }
...
static void test_for_each_mut_writes_through_the_view() {
    ...
    TRS_SPAN_FOR_EACH_MUT_AS(int32_t, elem, s) {
        *elem *= 10;
    }
...
static void test_mut_to_span_preserves_view() {
    const trs_SpanMut m = TRS_SPAN_OF_MUT(int32_t, 1, 2);
    const trs_Span s = trs_span_mut_to_span(m);
...
static void test_mut_to_span_shares_the_memory() {
    int32_t buf[3] = {1, 2, 3};
    const trs_SpanMut m = TRS_SPAN_FROM_DATA_MUT(int32_t, buf, 3);
    const trs_Span s = trs_span_mut_to_span(m);

    TRS_SPAN_SET(int32_t, m, 1, 99);

    TEST_ASSERT_EQUAL_INT32(99, *TRS_SPAN_GET_AS(int32_t, s, 1));
}

/* ========== of ========== */

static void test_of_views_the_literal() {
    const trs_Span s = TRS_SPAN_OF(int32_t, 3, 1, 2);

    TEST_ASSERT_EQUAL_size_t(3, s.len);
    TEST_ASSERT_EQUAL_size_t(sizeof(int32_t), s.elem_size);
    TEST_ASSERT_EQUAL_INT32(3, *TRS_SPAN_GET_AS(int32_t, s, 0));
    TEST_ASSERT_EQUAL_INT32(1, *TRS_SPAN_GET_AS(int32_t, s, 1));
    TEST_ASSERT_EQUAL_INT32(2, *TRS_SPAN_GET_AS(int32_t, s, 2));
}

static void test_of_derives_len_from_the_list() {
    TEST_ASSERT_EQUAL_size_t(1, TRS_SPAN_OF(int32_t, 42).len);
    TEST_ASSERT_EQUAL_size_t(5, TRS_SPAN_OF(int32_t, 1, 2, 3, 4, 5).len);
    TEST_ASSERT_EQUAL_size_t(sizeof(int64_t), TRS_SPAN_OF(int64_t, 1, 2).elem_size);
}
...
static void test_of_carries_struct_elems() {
    const trs_Span s = TRS_SPAN_OF(Pair, {1, 2}, {3, 4});

    TEST_ASSERT_EQUAL_size_t(2, s.len);
    TEST_ASSERT_EQUAL_size_t(sizeof(Pair), s.elem_size);
    TEST_ASSERT_EQUAL_INT64(1, TRS_SPAN_GET_AS(Pair, s, 0)->a);
    TEST_ASSERT_EQUAL_INT64(2, TRS_SPAN_GET_AS(Pair, s, 0)->b);
    TEST_ASSERT_EQUAL_INT64(3, TRS_SPAN_GET_AS(Pair, s, 1)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_SPAN_GET_AS(Pair, s, 1)->b);
}

// the literal behind a mut view is not const, so it takes writes
static void test_of_mut_is_writable() {
    const trs_SpanMut s = TRS_SPAN_OF_MUT(int32_t, 10, 20, 30);

    TRS_SPAN_SET(int32_t, s, 0, 99);
    *TRS_SPAN_GET_MUT_AS(int32_t, s, 2) = 77;
    trs_span_swap_elems(s, 0, 2);

    const trs_Span v = trs_span_mut_to_span(s);
    TEST_ASSERT_EQUAL_INT32(77, *TRS_SPAN_GET_AS(int32_t, v, 0));
    TEST_ASSERT_EQUAL_INT32(20, *TRS_SPAN_GET_AS(int32_t, v, 1));
    TEST_ASSERT_EQUAL_INT32(99, *TRS_SPAN_GET_AS(int32_t, v, 2));
}

static void test_of_mut_carries_struct_elems() {
    const trs_SpanMut s = TRS_SPAN_OF_MUT(Pair, {1, 2}, {3, 4});

    trs_span_swap_elems(s, 0, 1);

    TEST_ASSERT_EQUAL_INT64(3, TRS_SPAN_GET_MUT_AS(Pair, s, 0)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_SPAN_GET_MUT_AS(Pair, s, 0)->b);
    TEST_ASSERT_EQUAL_INT64(1, TRS_SPAN_GET_MUT_AS(Pair, s, 1)->a);
    TEST_ASSERT_EQUAL_INT64(2, TRS_SPAN_GET_MUT_AS(Pair, s, 1)->b);
}
...
static void test_bytes() {
    ...
    TEST_ASSERT_EQUAL_size_t(4 * sizeof(int32_t), trs_span_bytes(TRS_SPAN_FROM_DATA(int32_t, buf, 4)));
    TEST_ASSERT_EQUAL_size_t(0, trs_span_bytes(TRS_SPAN_FROM_DATA(int32_t, buf, 0)));
}

static void test_bytes_of_null_view_is_zero() {
    TEST_ASSERT_EQUAL_size_t(0, trs_span_bytes(TRS_SPAN_FROM_DATA(int32_t, nullptr, 0)));
}

// elem_size, not the elem count, drives the total
static void test_bytes_tracks_elem_size() {
    TEST_ASSERT_EQUAL_size_t(2 * sizeof(Pair), trs_span_bytes(TRS_SPAN_OF(Pair, {1, 2}, {3, 4})));
}

/* ========== access ========== */

static void test_get_reads_through() {
    const trs_Span s = TRS_SPAN_OF(int32_t, 10, 20, 30);

    TEST_ASSERT_EQUAL_INT32(10, *TRS_SPAN_GET_AS(int32_t, s, 0));
    TEST_ASSERT_EQUAL_INT32(30, *TRS_SPAN_GET_AS(int32_t, s, 2));
}
...
static void test_sub_offsets_and_shortens() {
    ...
    const trs_Span mid = trs_span_sub(s, 1, 3);
    ...
    TEST_ASSERT_EQUAL_INT32(1, *TRS_SPAN_GET_AS(int32_t, mid, 0));
    TEST_ASSERT_EQUAL_INT32(3, *TRS_SPAN_GET_AS(int32_t, mid, 2));

    // subspans compose: indices are relative to the span they are taken from
    const trs_Span inner = trs_span_sub(mid, 1, 1);
    TEST_ASSERT_EQUAL_size_t(1, inner.len);
    TEST_ASSERT_EQUAL_INT32(2, *TRS_SPAN_GET_AS(int32_t, inner, 0));
}

// the two bounds sub allows: the whole span, and the exact tail
static void test_sub_takes_the_whole_span_and_the_exact_tail() {
    const trs_Span s = TRS_SPAN_OF(int32_t, 0, 1, 2, 3);

    const trs_Span all = trs_span_sub(s, 0, 4);
    TEST_ASSERT_EQUAL_PTR(s.data, all.data);
    TEST_ASSERT_EQUAL_size_t(4, all.len);

    // start > 0 and start + count == len
    const trs_Span tail = trs_span_sub(s, 2, 2);
    TEST_ASSERT_EQUAL_size_t(2, tail.len);
    TEST_ASSERT_EQUAL_INT32(2, *TRS_SPAN_GET_AS(int32_t, tail, 0));
    TEST_ASSERT_EQUAL_INT32(3, *TRS_SPAN_GET_AS(int32_t, tail, 1));
}

// start == len is legal and yields an empty view, not an error
static void test_sub_at_end_is_empty() {
    const trs_Span s = TRS_SPAN_OF(int32_t, 0, 1, 2);

    const trs_Span tail = trs_span_sub(s, 3, 0);
...
static void test_sub_offsets_by_elem_size() {
    const trs_Span s = TRS_SPAN_OF(Pair, {1, 2}, {3, 4}, {5, 6});

    const trs_Span tail = trs_span_sub(s, 1, 2);
    TEST_ASSERT_EQUAL_size_t(2, tail.len);
    TEST_ASSERT_EQUAL_INT64(3, TRS_SPAN_GET_AS(Pair, tail, 0)->a);
    TEST_ASSERT_EQUAL_INT64(4, TRS_SPAN_GET_AS(Pair, tail, 0)->b);
    TEST_ASSERT_EQUAL_INT64(5, TRS_SPAN_GET_AS(Pair, tail, 1)->a);
    TEST_ASSERT_EQUAL_INT64(6, TRS_SPAN_GET_AS(Pair, tail, 1)->b);
}
...
static void test_sub_of_null_view_stays_null() {
    ...
    const trs_Span sub = trs_span_sub(s, 0, 0);
...
static void test_sub_mut_of_null_view_stays_null() {
    ...
    const trs_SpanMut sub = trs_span_sub_mut(s, 0, 0);
    TEST_ASSERT_NULL(sub.data);
    TEST_ASSERT_EQUAL_size_t(0, sub.len);
...
static void test_sub_mut_writes_reach_the_source() {
    ...
    const trs_SpanMut tail = trs_span_sub_mut(s, 2, 2);
    TRS_SPAN_SET(int32_t, tail, 0, 88);
...
static void test_mut_fprint_writes_what_fprint_writes() {
    ...
    assert_mut_prints("[1, 2, 3]\n", TRS_SPAN_FROM_DATA_MUT(int32_t, buf, 3));
    assert_mut_prints("[]\n", TRS_SPAN_FROM_DATA_MUT(int32_t, buf, 0));
}
...
static void test_print_twins_write_to_stdout() {
    ...
    trs_span_print(TRS_SPAN_FROM_DATA(int32_t, buf, 2), trs_fprint_i32);
    trs_span_mut_print(TRS_SPAN_FROM_DATA_MUT(int32_t, buf, 2), trs_fprint_i32);
}
