...
/// @ingroup ds
/// @brief trs_Stack — an owning last in first out stack
///
...
/// only: trs_stack_to_span is a view to read, and there is no mutable one to sort through.
///
...
/// An opaque handle: it comes from a constructor and goes back to trs_stack_drop
typedef struct trs_Stack trs_Stack;
...
/// @param self the stack to copy
/// @param al where the copy lives; trs_stack_copy is this one with the allocator of 'self'
/// @param[out] out the new stack, written only on success
/// @retval TRS_STATUS_OK on success
/// @retval TRS_STATUS_ERR_NO_MEM when the header or the block cannot be allocated
/// @bigo{n}
...
/// @param self asserts the stack is not empty. Read the elem with trs_stack_top first —
...
///       allocator with trs_stack_copy_with and hand the results over with
///       trs_stack_move_assign
/// @bigo{1}
TRS_API
...
/// trs_stack_new with sizeof(T) for the elem size
/// @param T the elem type
/// @param al the allocator
/// @param[out] out where the new stack is written
/// @bigo{1}
#define TRS_STACK_NEW(T, al, out) \
    trs_stack_new(sizeof(T), (al), (out))

/// trs_stack_new_cap with sizeof(T)
/// @param T the elem type
/// @param cap how many elems to make room for
/// @param al the allocator
/// @param[out] out where the new stack is written
/// @bigo{n}
#define TRS_STACK_NEW_CAP(T, cap, al, out) \
    trs_stack_new_cap((cap), sizeof(T), (al), (out))

/// trs_stack_from_data with sizeof(T)
/// @param T the elem type
...
/// @param[out] out where the new stack is written
/// @bigo{n}
#define TRS_STACK_FROM_DATA(T, data, len, al, out) \
    trs_stack_from_data((const T *){ (data) }, (len), sizeof(T), (al), (out))

/// a new stack from the elems written out: TRS_STACK_OF(int32_t, al, &s, 5, 3, 1)
/// @param T the elem type
/// @param al the allocator
/// @param[out] out where the new stack is written
/// @param ... the elems, bottom to top, as a T initializer list
/// @bigo{n}
#define TRS_STACK_OF(T, al, out, ...)                   \
    trs_stack_from_data(                                \
        (const T[]){ __VA_ARGS__ },                     \
        sizeof((const T[]){ __VA_ARGS__ }) / sizeof(T), \
        sizeof(T), (al), (out))

/// trs_stack_top as a const T *
/// @param T the elem type
/// @param self the stack
/// @bigo{1}
#define TRS_STACK_TOP_AS(T, self) \
    ((const T *) trs_stack_top((self)))

/// trs_stack_top_mut as a T *
/// @copydetails TRS_STACK_TOP_AS
#define TRS_STACK_TOP_MUT_AS(T, self) \
    ((T *) trs_stack_top_mut((self)))

/// trs_stack_push from a value rather than an address
/// @param T the elem type; a scalar, since 'val' becomes a compound literal
/// @param self the stack
/// @param val the value to copy in
/// @bigo{1} amortized
#define TRS_STACK_PUSH(T, self, val) \
    trs_stack_push((self), &(T){ (val) })
