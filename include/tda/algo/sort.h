...
///
/// tda_span_sort is the default: a quicksort, ninther pivot, three-way split, in place
/// and allocating nothing. It is not stable — stability costs a buffer, so it is a
/// separate name that says so by taking an allocator and returning a tda_Status.
///
...
/// @retval TDA_STATUS_ERR_NO_MEM when 'al' cannot give it; 's' is left as it was
/// @bigo{n log n}
[[nodiscard]] TDA_API
...
///         prefix before it is always sorted, which is what tda_span_inplace_merge wants
/// @bigo{n}
[[nodiscard]] TDA_API
