...
int main() {
    ...
    if (TRS_STATUS_IS_ERR(TRS_PQUEUE_OF(int32_t, trs_cmp_i32, al, &q, 3, 1, 4, 1, 5))) {
        return 1;
    }

    // what a print shows is heap order, not sorted order: only the front is in its final
    // place
    trs_pqueue_print(q, trs_fprint_i32); // [5, 3, 4, 1, 1]
    printf("%" PRId32 "\n", *TRS_PQUEUE_TOP_AS(int32_t, q)); // 5
    /// [build]

    /// [serve]
    // a push finds its place in O(log n), and the front is whatever is greatest now
    int rc = 1;
    trs_Vec *v = nullptr; // taken from a queue further down, and named here for the exit

    if (TRS_STATUS_IS_ERR(TRS_PQUEUE_PUSH(int32_t, q, 9))) {
        goto out;
    }
    printf("%" PRId32 "\n", *TRS_PQUEUE_TOP_AS(int32_t, q)); // 9
    ...
    while (trs_pqueue_len(q) > 0) {
        printf("%" PRId32 " ", *TRS_PQUEUE_TOP_AS(int32_t, q));
        trs_pqueue_pop(q);
    }
    putchar('\n'); // 9 5 4 3 1 1
    /// [serve]
    ...
    trs_PQueue *least = nullptr;
    if (TRS_STATUS_IS_ERR(TRS_PQUEUE_OF(int32_t, trs_cmp_desc_i32, al, &least, 3, 1, 4))) {
        goto out;
    }
    printf("%" PRId32 "\n", *TRS_PQUEUE_TOP_AS(int32_t, least)); // 1
    ...
    const trs_Span view = trs_pqueue_to_span(least);
    printf("%zu elems, the least of them %" PRId32 "\n", view.len,
           *TRS_SPAN_GET_AS(int32_t, view, 0)); // 3 elems, the least of them 1
    /// [order]
    ...
    trs_span_sort_heap(trs_vec_to_span_mut(v), trs_cmp_desc_i32);
    trs_vec_print(v, trs_fprint_i32); // [4, 3, 1]
