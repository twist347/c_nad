...
int main() {
    /// [build]
    trs_Al *pool = trs_al_pool_new(trs_al_default(), sizeof(int32_t), 3);
    if (!pool) {
        return 1;
    }
    ...
    for (size_t i = 0; i < st.block_count; ++i) {
        p = trs_alloc(pool, sizeof(int32_t));
    }
    printf("%zu free after taking them all\n", trs_al_pool_stats(pool).free); // 0

    // null now means the blocks ran out
    printf("%s\n", trs_alloc(pool, sizeof(int32_t)) ? "served" : "null: none left");

    // and null still means that after one comes back, if the request does not fit a block
    trs_dealloc(pool, p, sizeof(int32_t));
    printf("%s\n", trs_alloc(pool, st.block_size + 1) ? "served" : "null: over a block");
    /// [limits]
